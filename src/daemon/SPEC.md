...
启动流程按顺序完成：加载 .env → 初始化 SqliteStorage（失败则 start() 返回 Err）→ 初始化 JsonSessionConfigProvider（文件不存在则 warn + 硬编码默认值）→ 创建 ArchiveSweeper 并 spawn 后台任务 → 初始化 PermissionEngine → 初始化 AgentRegistry → 初始化 Gateway（注入 storage）→ 注册 Feishu 适配器 → 启动 ShutdownCoordinator → 注册 LLM Providers（凭据文件优先、env var fallback）→ 启动 Chat TCP Server（传入 config_dir）→ 启动 AuditLogger。

SqliteStorage 的 data_dir 直接使用 `config_dir` 参数，不做 fallback 计算。

关闭流程采用状态机管理：Running → ShuttingDown → Draining → Stopped，等待所有在途操作完成后退出，超时强制终止；Daemon shutdown 时通过 `sweeper_shutdown_tx` 通知 ArchiveSweeper 优雅退出；所有活跃 session 在进程退出前通过 `flush_all_sessions()` 将 checkpoint 同步持久化到存储后端。

## 公开接口

### 构造

- **`Daemon::start(config_dir)`** — 异步构造器，按启动流程初始化所有组件（读取 `BOOTSTRAP_MODE` 环境变量确定 bootstrap 模式，传入 `SessionManager::new`；将 `config_dir` 作为 workspace_dir 传入），返回完整的 Daemon 实例
...
- **`Daemon::read_bootstrap_mode()`** — 读取 `BOOTSTRAP_MODE` 环境变量并转换为 `BootstrapMode`（`"minimal"` → Minimal，其它包括缺省 → Full）

### 主操作

- **`Daemon::run()`** — 阻塞当前协程直到收到 SIGINT/SIGTERM 信号，然后触发优雅关闭（Unix 平台监听两种信号，非 Unix 平台仅支持 Ctrl+C）；关闭时先执行 `initiate_shutdown()` 等待 drain 完成，再调用 `flush_all_sessions()` 持久化所有活跃 session 的 checkpoint，最后通知 ArchiveSweeper 退出
...
**`Daemon::init_llm_registry` 测试**（unit_tests.rs）

| 测试 | 描述 |
|------|------|
| `test_init_llm_registry_credentials_file_priority` | credentials 文件有 api_key 时能注册 provider |
| `test_init_llm_registry_env_fallback` | credentials 文件无但 env var 有时能注册 |
| `test_init_llm_registry_both_absent_no_registration` | 两者都无时不注册 |
