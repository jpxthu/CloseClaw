...
| `SessionManager::flush_all` | 遍历所有活跃 session，将 checkpoint 同步写入持久化后端；单个失败只 warn 不中断；storage 未配置时返回 Ok(0) |
...
`SessionManager` 还持有 `conversation_sessions: RwLock<HashMap<String, Arc<RwLock<ConversationSession>>>>`，在 `find_or_create` 创建 session 时同步创建对应的 `ConversationSession`。通过 `get_conversation_session`、`is_session_busy`、`push_pending_message`、`pop_pending_message` 方法提供 busy/pending 管理。
...
- **idle 状态**：设置 busy，spawn 异步任务调用 `FallbackClient::chat()`，消费响应文本，收到 `MessageEnd` 或 stream 结束时清除 busy 并检查 pending
