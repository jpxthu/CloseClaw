...
| 接口 | 功能 |
|------|------|
| `Gateway::route_message` | 将消息路由到对应 channel 的适配器，自动创建会话 |
...
`Gateway` 内部维护 `HashMap<String, Session>`（key = `"channel:to"`）。`route_message` 在首次收到某 channel+to 的消息时自动创建 Session。
