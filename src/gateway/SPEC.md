...
| 接口 | 功能 |
|------|------|
| `Gateway::new` | 创建 Gateway 实例（无 processor_registry） |
...
| `Gateway::send_outbound` | 将 agent 原始输出经 processor chain 处理后按 `msg_type` 分发到适配器（text→send_message，interactive→send_card_json） |
...
### 3.4 processor_registry 集成

`Gateway` 持有 `processor_registry: Option<Arc<ProcessorRegistry>>`（由 `with_processor_registry` 注入）。
...
**出站消息**：在 `send_outbound` 中处理：

1. 若 `processor_registry` 为 `None` → 直接将 `raw_output` 作为 text 消息发送（bypass）
2. 若 `processor_registry` 存在 → 构造 `ProcessedMessage { content: raw_output, .. }`，调用 `process_outbound`
3. 若返回的 `suppress == true` → 不发送任何消息，直接返回
4. 解析 processed content JSON，读取 `msg_type`：
   - `"text"` → 从 `content` 字段取文本，构造 `Message` 调用 `adapter.send_message`
   - `"interactive"` → 从 `content` 字段取 JSON 字符串，调用 `adapter.send_card_json`
   - 其他或解析失败 → fallback 为普通文本消息

此设计保证：默认（未配置 `processor_chain`）Gateway 行为不受影响。
