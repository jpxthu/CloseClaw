...
| 接口 | 功能 |
|------|------|
| `Gateway::set_storage` | 配置持久化存储后端，使 Gateway 具备 archived session 恢复能力 |
...
### 4.2 Archived Session 恢复

当 `storage` 被配置后，`route_message` 在创建新 Session 前会调用 `try_restore_archived_session` 检查存储中是否存在该 session_id 的 Archived checkpoint：若 status 为 Archived，则通过对应 channel 的 adapter 发送 "正在恢复会话..." 通知，调用 `storage.restore_checkpoint` 恢复 session，恢复后重新加载 checkpoint，并用 checkpoint 的 `chat_id` 填充新 Session 的 `agent_id` 字段（Gateway Session 仅含 id / agent_id / channel / created_at，checkpoint 的 mode_state、messages 等数据不在 Session 结构中直接体现）。若 status 为 Active 或无存储记录，则保持现有行为（创建新内存 session）。通知发送失败仅 warn，不阻塞消息路由。

### 4.3 Feishu Token 缓存

Feishu 的 tenant_access_token 有效期约 2 小时，`FeishuAdapter` 在 `Arc<Mutex<Option<CachedToken>>>` 中缓存，**提前 5 分钟主动刷新**（1.5h 后触发）。

### 4.4 FeishuAdapter HTTP Client
