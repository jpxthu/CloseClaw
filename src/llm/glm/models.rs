...
use crate::llm::{ModelInfo, ModelLister};

use super::types::{GlmModelsResponse, GlmModel};
...
fn build_model_info(m: GlmModel) -> ModelInfo {
    ...
    let kb =
        crate::llm::ProviderModelKnowledge::new();
...
impl GlmProvider {
    ...
    > {
        ...
        .await
        {
            Ok(Ok(resp)) => resp,
            Ok(Err(e)) => {
                return Err(ProviderError::Reqwest(e))
            }
            Err(_) => {
        ...
        let api_resp: GlmModelsResponse = response
            .json()
            .await
