...
/// Same category entries → negative_signal = 0.0, score unaffected by negative_signal_weight.
#[test]
...
async fn test_run_once_diary_only_promoted_groups() {
    ...
    let llm: std::sync::Arc<dyn DreamingLlmCaller> =
        std::sync::Arc::new(MockDiaryLlm);
