...
//! [`llm::debug_log`](closeclaw_llm::debug_log), and
...
/// together, reducing the field count in [`SlashEmitEventParams`].
pub struct SlashDebugLogContext<'a> {
...
/// Parameters for emitting a slash debug log event.
///
/// Aggregates all event fields into a struct to keep
/// [`emit_slash_event`] within the project's 6-parameter limit.
pub struct SlashEmitEventParams<'a> {
    /// Bundled debug-log context (instance, trace ID, and session key).
    pub ctx: SlashDebugLogContext<'a>,
    /// Log level for the event.
    pub level: LogLevel,
    /// Source module that produced the event.
    pub source_module: &'a str,
    /// Event type identifier (e.g. `"slash.command"`).
