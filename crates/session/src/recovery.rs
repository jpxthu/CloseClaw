...
/// Prefix marker for progress-related entries in `system_injection_appends`.
pub const PROGRESS_APPEND_PREFIX: &str = "__progress__:";
use std::sync::Arc;
use tokio::sync::RwLock;

/// Prefix marker for approval history entries in `system_injection_appends`.
///
...
/// `system_injection_appends` during recovery.
///
...
impl<S: PersistenceService + ?Sized> SessionRecoveryService<S> {
    ...
    pub async fn recover(&self) -> Result<RecoveryReport, PersistenceError> {
        ...
        for session_id in &recovered {
            if let Some(cp) = checkpoints.get_mut(session_id) {
                ...
                // Layer 3: inject approval tool call history into system_injection_appends
    ...
    /// Inject approval tool call history into checkpoint's system_injection_appends (layer 3).
    ///
    /// When a checkpoint has `approval_tool_calls`, this method formats them
    /// and adds them to `system_injection_appends` with [`APPROVAL_HISTORY_PREFIX`]
    ...
    fn inject_approval_history(&self, session_id: &str, checkpoint: &mut SessionCheckpoint) {
        ...
        tracing::info!(
            ...
            "injected approval tool call history into system_injection_appends"
        );
    }

    /// Inject the plan file Tasks section into checkpoint's system_injection_appends.
    ///
    /// For sessions in Executing or Paused state with a `plan_state`,
    /// extracts the Tasks section from the plan file and adds it to
    /// `system_injection_appends` with [`PLAN_TASKS_PREFIX`].
    ///
    ...
    fn inject_plan_references(&self, session_id: &str, checkpoint: &mut SessionCheckpoint) {
        ...
        tracing::info!(
            ...
            "layer 4 fallback: injected plan references into system_injection_appends"
        );
    }
