...
#[tokio::test]
async fn test_set_mode_initial_input_and_no_input() {
    // Case 1: with initial_input → pending message pushed.
    let mock = Arc::new(MockSlashEffectExecutor::new());
    ...
    // Case 2: without initial_input → no pending message.
    let mock2 = Arc::new(MockSlashEffectExecutor::new());
    let pending2 = Arc::new(Mutex::new(Vec::<PendingMessage>::new()));
    let sm2: Arc<dyn SessionLookup> = Arc::new(MockSessionLookup::with_pending(pending2.clone()));
    let ctx2 = make_ctx(Arc::clone(&mock2), "s4", "feishu", sm2);
    SlashResult::SetMode {
        mode: "auto".into(),
        plan_file_path: None,
        initial_input: None,
    ...
    .execute(&ctx2)
    .await;
    assert!(pending2.lock().unwrap().is_empty());
}
