...
//! error notifications as the underlying unified chat client produces
//! stream events. Implementations forward these to a downstream
//! transport (Feishu card update, CLI stdout, etc.) without the session
...
/// stream events arrive; it never inspects the concrete transport.
///
/// ## Contract
///
