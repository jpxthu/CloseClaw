...
impl DslParseResult {
    /// Construct a [`DslParseResult`] from a slice of [`ContentBlock`].
    ///
    /// Only [`ContentBlock::Text`] variants are processed; other block
    /// types are skipped. Text contents are concatenated with newlines
    /// before parsing.
    ...
    /// Parse DSL instructions from a raw text string.
    ///
    /// Recognizes `::button[...]` and `::selector[...]` DSL lines.
    fn parse_from_text(text: &str) -> Self {
        let mut instructions: Vec<DslInstruction> = Vec::new();
        for line in text.lines() {
            if let Some(inst) = Self::parse_dsl_line(line) {
    ...
    fn parse_dsl_line(line: &str) -> Option<DslInstruction> {
        ...
        if trimmed.starts_with("::button[") {
            return Self::parse_bracket_instruction(trimmed, "button");
        }
        if trimmed.starts_with("::selector[") {
            return Self::parse_bracket_instruction(trimmed, "selector");
        }
        None
    }

    fn parse_bracket_instruction(trimmed: &str, instr_type: &str) -> Option<DslInstruction> {
