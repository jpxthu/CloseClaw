//! Unit tests for SlashResult::execute.
...
use crate::session_types::ReasoningLevel;
use crate::slash_router::{
...
#[tokio::test]
async fn test_set_mode_sends_confirmation_and_no_executor_call() {
    let (ctx, mut rx, _exec) = make_ctx();
    ...
    // SetMode does not invoke any executor method — nothing to assert
}
