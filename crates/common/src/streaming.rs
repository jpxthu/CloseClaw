...
///
/// Allows plugins to store the renderer behind a `Mutex` and call
/// `handle_event` / `flush` through a shared reference.
impl StreamingRenderer for std::sync::Mutex<DefaultStreamingRenderer> {
    fn handle_event(&mut self, event: StreamEvent) -> StreamingOutput {
        self.get_mut()
