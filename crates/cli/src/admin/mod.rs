//! CLI admin module — handler functions for admin subcommands.

mod agent;
mod common;
mod config;
mod restart;
pub mod rpc;
mod rule;
mod run;
mod skill;
mod stop;

pub use rpc::{
    admin_socket_path, AdminClient, AdminContext, AdminRequest, AdminResponse, AdminServer,
};

pub use agent::{handle_agent, handle_agent_with};
pub use common::{
    mask_key, ConfigListFile, ConfigListOutput, ConfigValidateOutput, RuleCheckOutput,
    RuleListEntry, RuleListOutput, RunOutput, StopOutput,
};
pub use config::{handle_config, handle_config_with, read_config_files};
pub use restart::handle_restart;
pub use rule::{handle_rule, handle_rule_with};
pub use run::{handle_run, handle_run_foreground, prepare_run, DaemonRunner};
pub use skill::{handle_skill, handle_skill_with};
pub use stop::{handle_stop, handle_stop_at, handle_stop_at_with_timeout, STOP_TIMEOUT};

#[cfg(test)]
mod config_tests;
#[cfg(test)]
mod run_tests;
#[cfg(test)]
mod stop_tests;
