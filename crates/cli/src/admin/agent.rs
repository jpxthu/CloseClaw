...
async fn handle_agent_info_rpc(client: &AdminClient, id: &str, json: bool) -> Result<()> {
    ...
    match resp {
        AdminResponse::AgentInfoResult(info) => {
            ...
            print_subagents(&info.subagents);
...
fn print_identity(info: &AgentInfoResult) {
...
fn print_runtime(info: &AgentInfoResult) {
    println!(
        "  Model: {}",
        info.model
            .as_ref()
...
fn print_capabilities(info: &AgentInfoResult) {
...
fn print_subagents(sub: &SubagentsConfig) {
    println!("  Subagents:");
...
fn print_memory(memory: &Option<MemoryConfig>) {
    match *memory {
        Some(ref mem) => {
            println!("  Memory:");
