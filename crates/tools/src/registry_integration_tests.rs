...
/// SkillTool (deferred) and SkillCreatorTool (deferred) appear in
/// separate groups in the first-level index. Both groups show
/// "(deferred)" since SkillTool was changed to deferred per design doc.
#[tokio::test]
async fn test_index_builder_skill_tool_eager_skill_creator_deferred() {
    ...
    // skills group has SkillTool (deferred) → "(deferred)"
    assert!(
        section.contains("**skills** — (deferred)"),
        "skills group should be deferred, got: {section}"
    );
    // SkillTool appears without bold (deferred)
    assert!(
        section.contains("  - SkillTool"),
        "SkillTool should appear without bold (deferred), got: {section}"
    );
    assert!(
        !section.contains("**SkillTool**"),
        "SkillTool should NOT be bold (deferred), got: {section}"
    );
