//! Debug log helpers for the Tools module.
//!
//! Provides structured debug-log emission for tool execution lifecycle events:
//! tool execution start/end, full parameters/return values, and permission
//! check results.
//!
//! Follows the same pattern as
//! [`gateway::debug_log_emitter`](closeclaw_gateway::debug_log_emitter),
//! [`session::debug_log`](closeclaw_session::debug_log), and
...
/// together, reducing the field count in [`ToolsEmitEventParams`].
pub struct ToolsDebugLogContext<'a> {
...
/// Parameters for emitting a tools debug log event.
///
/// Aggregates all event fields into a struct to keep
/// [`emit_tool_event`] within the project's 6-parameter limit.
pub struct ToolsEmitEventParams<'a> {
    /// Bundled debug-log context (instance, trace ID, and session key).
    pub ctx: ToolsDebugLogContext<'a>,
    /// Log level for the event.
    pub level: LogLevel,
    /// Source module that produced the event.
    pub source_module: &'a str,
    /// Event type identifier (e.g. `"tool.execution.start"`).
...
/// Emit a structured debug log event for the Tools module.
///
...
/// no-op.
pub fn emit_tool_event(params: ToolsEmitEventParams<'_>) {
...
mod tests {
    ...
    fn empty_trace_id_is_noop() {
        ...
        let ctx = ToolsDebugLogContext::new(None, "", None);
    ...
    fn none_debug_log_is_noop() {
        ...
        let ctx = ToolsDebugLogContext::new(None, "trace-123", None);
        emit_tool_event(ToolsEmitEventParams {
            ctx,
            level: LogLevel::Info,
            source_module: "tools",
    ...
    fn debug_log_context_new_fields() {
        let ctx = ToolsDebugLogContext::new(None, "tid", Some("skey"));
