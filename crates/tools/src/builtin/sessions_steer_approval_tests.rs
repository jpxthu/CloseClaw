...
#[tokio::test]
async fn test_steer_approval_required_routes_through_approval_flow() {
    let flow = make_approval_flow();
    ...
    // 4. Now verify the steer tool also correctly routes a Denied response
    ...
    let result = tool
        .call(
            json!({"sessionId": "child-deny-route", "task": "new task"}),
            &ctx2,
        )
        .await;
    ...
    assert_eq!(
        output.data["status"], "approval_pending",
        "denied steer should route to approval_pending"
    );
    assert!(
        output.data["request_id"].is_string(),
