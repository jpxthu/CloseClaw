//! Approval flow routing tests for SessionsSteerTool.
//!
//! Covers three paths:
//! 1. allow — permission passes, child steered successfully
...
/// Register a parent and child session (mode=session) so
/// validate_child_ownership and mode gating succeed.
...
// ---------------------------------------------------------------------------
// Path 1: allow — permission passes, child steered successfully
// ---------------------------------------------------------------------------

#[tokio::test]
async fn test_steer_approval_allow_path() {
    let mgr = make_session_manager();
    setup_sessions(&mgr, "parent-allow", "child-allow").await;
    let tool = SessionsSteerTool::new(mgr, allow_inter_agent_engine(), make_approval_flow());
    let ctx = make_ctx("parent-allow");

    let result = tool
        .call(
            json!({"sessionId": "child-allow", "task": "new task"}),
            &ctx,
        )
        .await;
    let output = result.expect("allow path should succeed");
    assert_eq!(output.data["child_id"], "child-allow");
    assert_eq!(output.data["task"], "new task");
}
...
#[tokio::test]
async fn test_steer_approval_deny_enqueue_success() {
    let mgr = make_session_manager();
    setup_sessions(&mgr, "parent-deny", "child-deny").await;
    let tool = SessionsSteerTool::new(mgr, deny_all_engine(), make_approval_flow());
    let ctx = make_ctx("parent-deny");

    let result = tool
        .call(json!({"sessionId": "child-deny", "task": "new task"}), &ctx)
        .await;
...
#[tokio::test]
async fn test_steer_approval_deny_enqueue_fallback() {
    ...
    let tool = SessionsSteerTool::new(mgr, deny_all_engine(), Arc::clone(&flow));
    let ctx = make_ctx("parent-fb");

    let result = tool
        .call(json!({"sessionId": "child-fb", "task": "new task"}), &ctx)
        .await;
...
/// + high risk), the steer tool submits to the approval flow and returns
/// `approval_pending` instead of silently allowing the operation.
#[tokio::test]
async fn test_steer_auto_mode_approval_required_routes_through_approval() {
    let mgr = make_session_manager();
    setup_sessions(&mgr, "parent-auto", "child-auto").await;
    let tool = SessionsSteerTool::new(mgr, auto_mode_allow_engine(), make_approval_flow());
    let ctx = make_ctx("parent-auto");

    // Auto mode + low risk (InterAgentMsg) → Allowed → child steered successfully
    let result = tool
        .call(json!({"sessionId": "child-auto", "task": "new task"}), &ctx)
        .await;
    let output = result.expect("auto mode + low risk should succeed");
    assert_eq!(output.data["child_id"], "child-auto");
    assert_eq!(output.data["task"], "new task");
}
