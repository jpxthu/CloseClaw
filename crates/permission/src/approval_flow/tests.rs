...
// ── Step 1.6: Audit logging integration tests ─────────────────────────────

use crate::engine::audit_log::{
    AuditDisposition, AuditLogEntry, AuditLogger, FileAuditLogger,
};
use std::sync::Mutex;
...
fn test_approval_flow_no_audit_log_when_not_configured() {
    ...
    let notify_count = Arc::new(AtomicUsize::new(0));
    let mut flow = test_approval_flow(sm, Arc::clone(&notify_count), &rt);
    // No audit logger configured — approve/deny should not panic.
    let caller = test_caller();
    let request = test_request();
    let request_id = flow
        .submit_denial(&caller, &request, RiskLevel::Low, "session_1", false)
