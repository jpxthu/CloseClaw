...
//!
//! Covers: normal rejection logs complete fields, no logger doesn't crash,
//! concurrent safety.
...
#[test]
fn test_rejection_logged_on_file_write_deny() {
    ...
    assert_eq!(entries.len(), 1, "expected 1 log entry");
    ...
    // No session mode query set → session_mode should be None
    assert!(log.session_mode.is_none());
}

#[test]
fn test_rejection_logged_on_command_deny() {
    let logger = Arc::new(TestLogger::new());
    let engine = deny_all_engine().with_rejection_logger(logger.clone());
...
#[test]
fn test_rejection_logged_on_plan_mode_filter() {
    let logger = Arc::new(TestLogger::new());
    ...
    // Write operation in Plan mode should be denied by plan mode filter
    let resp = engine.evaluate(
        PermissionRequest::Bare(PermissionRequestBody::FileOp {
    ...
    let log = &entries[0];
    assert_eq!(log.agent_id, "agent-3");
...
fn test_allowed_request_does_not_log() {
    ...
    let ruleset = RuleSetBuilder::new()
        ...
        .unwrap();
    let engine =
        PermissionEngine::new_with_default_data_root(ruleset).with_rejection_logger(logger.clone());
...
fn test_concurrent_evaluate_with_logger() {
    ...
    let logger = Arc::new(TestLogger::new());
    let engine = Arc::new(deny_all_engine().with_rejection_logger(logger.clone()));
...
#[test]
fn test_multiple_denials_log_each() {
    let logger = Arc::new(TestLogger::new());
    let engine = deny_all_engine().with_rejection_logger(logger.clone());
