...
fn decide_end_to_end_from_dir() {
    ...
    let feat = features("gpt-4o-basic", "hello world");
    let outcome = engine.decide(&feat);
    match outcome {
        DecisionOutcome::Decision(d) => {
...
fn decide_fixture_fallback_basic() {
    ...
    let feat = features("gpt-4o-basic-fallback", "something else");
    let outcome = engine.decide(&feat);
    match outcome {
        DecisionOutcome::Decision(d) => {
...
fn decide_fixture_error_injection_rate_limit() {
    ...
    let feat = features("gpt-4o-error", "hi");
...
fn decide_fixture_multi_turn_turn1() {
    ...
    let feat = features("gpt-4o-multi", "start");
    let outcome = engine.decide(&feat);
    match outcome {
        DecisionOutcome::Decision(d) => {
...
fn decide_fixture_multi_turn_turn2() {
    ...
    let feat1 = features("gpt-4o-multi", "start");
...
fn decide_fixture_usage_response() {
    ...
    let feat = features("gpt-4o-usage", "hi");
    let outcome = engine.decide(&feat);
    match outcome {
        DecisionOutcome::Decision(d) => {
...
fn decide_unknown_model_returns_default() {
    ...
    let feat = features("unknown-model", "hi");
    let outcome = engine.decide(&feat);
    match outcome {
        DecisionOutcome::Decision(d) => {
            assert_eq!(d.scenario, "default");
...
/// Request 1: cache_fields_missing=true (state machine records prefix).
/// Request 2: cache_fields_missing=false, same prefix → cache hit.
#[test]
fn state_machine_continuity_after_cache_fields_missing() {
    let mut engine = ScenarioEngine::new(vec![ScenarioDeclaration {
        name: "missing-then-auto".to_string(),
        match_: None,
        turns: vec![
            // Turn 0: cache_fields_missing=true
            TurnResponse {
                response: ResponseShape::Text(TextResponse {
            ...
            // Turn 1: cache_fields_missing=false (auto-sim)
            TurnResponse {
                response: ResponseShape::Text(TextResponse {
    ...
    // Request 1: cache_fields_missing=true, auto-sim produces values
    // but merge_cache_into_usage skips filling.
    let feat1 = features_with_messages(
        "gpt-4",
        vec![
            ("system", "sys"),
            ("user", "hello"),
    ...
    let d1 = match engine.decide(&feat1) {
        DecisionOutcome::Decision(d) => d,
        _ => panic!("expected decision"),
    };
    let u1 = d1.usage.unwrap();
    // cache_fields_missing=true → cache fields are NOT filled
    assert!(u1.cache_hit_tokens.is_none());
    assert!(u1.cache_write_tokens.is_none());

    // Request 2: cache_fields_missing=false, same prefix → auto-sim hit
    let feat2 = features_with_messages(
        "gpt-4",
        vec![
            ("system", "sys"),
    ...
    let d2 = match engine.decide(&feat2) {
        DecisionOutcome::Decision(d) => d,
        _ => panic!("expected decision"),
    };
    let u2 = d2.usage.unwrap();
    // State machine tracked the prefix in request 1, so request 2 gets a hit
    assert!(u2.cache_hit_tokens.is_some());
    assert!(u2.cache_hit_tokens.unwrap() > 0);
    assert!(u2.cache_write_tokens.is_none());
}
...
fn state_machine_continuity_break_after_cache_fields_missing() {
    let mut engine = ScenarioEngine::new(vec![ScenarioDeclaration {
        name: "missing-then-break".to_string(),
        match_: None,
        turns: vec![
            TurnResponse {
                response: ResponseShape::Text(TextResponse {
    ...
    // Request 2: cache_fields_missing=false, different prefix → break
    let feat2 = features_with_messages(
        "gpt-4",
        vec![
            ("system", "new sys"),
    ...
    let u2 = d2.usage.unwrap();
    // Different prefix → write tokens present
    assert!(u2.cache_write_tokens.is_some());
    assert!(u2.cache_write_tokens.unwrap() > 0);
}
