...
//! and [`session::debug_log`](closeclaw_session::debug_log).
...
pub struct LlmDebugLogContext<'a> {
...
/// Parameters for emitting an LLM debug log event.
///
/// Aggregates all event fields into a struct to keep
/// [`emit_llm_event`] within the project's 6-parameter limit.
pub struct LlmEmitEventParams<'a> {
    /// Bundled debug-log context (instance, trace ID, and session key).
    pub ctx: LlmDebugLogContext<'a>,
    /// Log level for the event.
    pub level: LogLevel,
    /// Source module that produced the event.
    pub source_module: &'a str,
    /// Event type identifier (e.g. `"llm.call.start"`).
