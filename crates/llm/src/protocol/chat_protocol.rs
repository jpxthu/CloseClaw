...
//! - Serialising an [`InternalRequest`][crate::InternalRequest] into a
//!   protocol-specific HTTP request body (`serde_json::Value`).
//! - Deserialising a protocol-specific HTTP response body into an
//!   [`InternalResponse`][crate::InternalResponse].
//! - Decorating an HTTP request with protocol-specific headers.
//! - Parsing raw SSE event streams into structured [`StreamEvent`][crate::StreamEvent] values.
//!
...
//! [`ProtocolId`][crate::ProtocolId] returned by a [`Provider`][crate::Provider].
...
///   [`IncomingSseStream`] (from a [`Provider`][crate::Provider]'s
///   [`send_streaming`][crate::Provider::send_streaming]) and produces a
///   stream of parsed [`StreamEvent`][crate::StreamEvent] values.
#[async_trait]
pub trait ChatProtocol: Send + Sync {
    ...
    ///
    /// The returned value must match one of the [`ProtocolId`][crate::ProtocolId]
    /// values advertised by the [`Provider`][crate::Provider] that hosts
    /// this protocol.
    ...
    /// [`Provider::supported_protocols`][crate::Provider::supported_protocols]
    ...
    /// parsed into an [`InternalResponse`][crate::InternalResponse].
    ...
    /// [`Provider::send_streaming`][crate::Provider::send_streaming]) and
