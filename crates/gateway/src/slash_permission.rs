...
async fn gw_stop(sm: &SessionManager, session_id: &str, cascade: bool, force: bool) {
    let mode = if force {
...
async fn gw_new_session(sm: &SessionManager, session_id: &str, channel: &str) -> String {
...
async fn gw_exec(command: &str) -> Vec<ContentBlock> {
    ...
    async fn execute_set_mode(&self, session_id: &str, mode: &str) {
        ...
        .await
        {
            match closeclaw_common::SessionMode::from_str_opt(mode) {
                Some(parsed) => cs.write().await.set_session_mode(parsed),
                None => tracing::warn!(
                    session_id,
                    mode,
