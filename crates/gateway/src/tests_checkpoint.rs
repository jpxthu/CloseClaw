...
use closeclaw_common::im_plugin::{AdapterError, IMAdapter};
...
fn make_gw_with_cm(
    ...
    crate::Gateway::new(config, sm).with_checkpoint_manager(cm)
}
...
async fn test_with_checkpoint_manager_stores_cm() {
    ...
    let cm = Arc::new(CheckpointManager::new(persistence.storage()));
    let gw = crate::Gateway::new(config, Arc::clone(&sm)).with_checkpoint_manager(cm);
