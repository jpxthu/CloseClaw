...
/// Helper: create a ConfigManager pre-populated with agent-b config.
fn make_test_config_manager() -> Arc<closeclaw_config::manager::ConfigManager> {
...
/// Create a SessionManager wired with all injection deps.
/// Returns (mgr, tmp) — caller must keep tmp alive.
async fn setup_mgr_with_injection_deps(
    ...
    let tmp = set_injection_deps(&mgr).await;
    (mgr, tmp)
}

/// Wire all injection dependencies onto an existing SessionManager.
/// Returns the TempDir that backs the workspace prompt builder.
async fn set_injection_deps(mgr: &SessionManager) -> tempfile::TempDir {
    let tmp = make_temp_workspace(&[
...
// ── Helper: pre-populate manager state for archived-restore tests ──

/// Insert routing key, session entry, and ConversationSession into the
/// manager so that `find_or_create` takes the needs_conv = false path.
async fn pre_populate_archived_session(mgr: &SessionManager, session_id: &str, routing_key: &str) {
    {
        let mut reg = mgr.key_registry.write().await;
    ...
    {
        let mut cs = mgr.conversation_sessions.write().await;
        cs.insert(session_id.to_string(), Arc::new(RwLock::new(conv)));
    }
}

/// Assert that all injection deps were wired by `rebuild_archived_session_prompt`.
async fn assert_injection_deps_wired(mgr: &SessionManager, session_id: &str) {
    let cs = mgr.get_conversation_session(session_id).await.unwrap();
    let cs = cs.read().await;
    ...
    assert!(
        ...
        "prompt should contain workspace content after full \
         injection: {}",
        prompt
    );
...
async fn test_archived_restore_full_injection_chain() {
    ...
    pre_populate_archived_session(&mgr, &session_id, &routing_key).await;

    let resolved = mgr.find_or_create("feishu", &msg, None).await.unwrap();
    assert_eq!(resolved, session_id);

    assert_injection_deps_wired(&mgr, &session_id).await;
}
