...
impl SessionManager {
    ...
    async fn reload_and_rebuild_restored_session(
        ...
        // Read straight from storage, bypassing the CM local cache:
        // restore_migrating_on_timeout has just flipped the checkpoint back
        // to Active via raw storage, but the cache may still pin the stale
        // Migrating value written by the earlier status check — the rebuild
        // must use the authoritative post-restore checkpoint.
        if let Some(cp) = cm
            .storage()
    ...
    ///
    /// Reads deliberately bypass the CheckpointManager local cache
    /// (`cm.storage().load_checkpoint` instead of `cm.load`): archiving is
    /// performed by the Sweeper directly on raw storage and never invalidates
    /// the CM cache, so a cache-first read would keep returning the stale
    /// Migrating checkpoint and the poll would always burn the full 30s
    /// deadline. Storage is the only authoritative status source while
    /// polling.
