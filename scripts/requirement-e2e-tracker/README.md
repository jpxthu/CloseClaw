...
跟踪各模块「测试用例发现」状态。当一个模块的需求文档（`docs/requirements/<module>.md`）或设计文档（`docs/design/<module>/` 下所有 `.md`）发生变更时，该模块的测试用例应被标记为「需重新发现」。

## 跟踪单位：模块

一个**模块**由以下文档组成：

- `docs/requirements/<module>.md`（`README.md`、`STANDARDS.md` 不是模块，会被排除）
- `docs/design/<module>/` 下所有 `.md`（递归）

模块清单从 `docs/requirements/*.md` 的文件名推导。任一跟踪文档自上次确认后发生变更，该模块即视为「需重新发现」。
...
扫描所有模块，报告「需求/设计文档自上次确认后变更」或「未跟踪」的模块。对每个模块的处理逻辑：
...
`ret` 与 `scripts/design-doc-tracker/ddt.py`（设计文档实现状态跟踪）共用 `scripts/_tracker_core.py` 中的 git helpers、records 读写、merge-base、diff 检测、`.md` 收集/排序等核心逻辑。两者跟踪单位不同：ddt 跟踪单个 `.md` 文件，ret 跟踪模块（需求 + 设计文档集合）。
