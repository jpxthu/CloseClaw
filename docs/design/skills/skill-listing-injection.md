...
清单生成仅在 SP 组装时触发，组装事件之间内容不变。技能文件变更在下一个组装边界反映——新建会话、恢复会话、压缩完成后的 SP 重建均从注册中心读取当前最新内容（组装触发事件见 [system_prompt 模块](../system_prompt/README.md)）。SP 重建时，SkillsFragmentProvider 从 Session 读取激活标记，将已激活的条件技能纳入清单一并渲染。
