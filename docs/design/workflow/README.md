...
Agent 调用 workflow_start 或用户输入 /workflow 指令触发。Engine 按优先级查找定义文件（agent workspace → .closeclaw/ → 内置），解析 yaml frontmatter 得到 Workflow 结构体。Engine 初始化 WorkflowRun（current_step=0，phase=executing），向 system prompt 追加区注入 workflow context，注入 Step 0 的 goal 消息（role: workflow），Agent 开始执行。

### 步骤执行循环

每个步骤经过三个阶段。Goal 阶段：Engine 注入步骤目标描述（role: workflow），Agent 收到后连续工具调用完成步骤。Verify 阶段：Agent turn 结束、session idle 时 Engine 注入验收清单，Agent 自查——未完成则继续执行等待下次 idle 再注入，完成则调用 workflow_verify() 声明完成。Jump 阶段：Engine 收到 verify 后注入跳转问题（含 ABCD 选项），Agent 通过 workflow_jump 回答，Engine 按 transitions 条件匹配决定动作。Engine 抹除 verify/jump 交互记录（含 tool_call 和 tool_result），更新 WorkflowRun 状态，注入下一步 goal。循环直到 complete。
...
Agent 未完成验证时可继续执行，Engine 等下次 session idle 自动重新注入 verify。超时死锁保护作为兜底：Engine 联系注入 verify 多次后 Agent 仍未响应，超过重试上限则标记为 blocked 并通知 owner。Agent 也可主动调用 workflow_blocked 请求阻塞。Blocking 类型的步骤同样由 owner 输入后恢复。

工作流正常结束后 Engine 清理：移除追加区中的 workflow context、清理 goal 消息、清空 WorkflowRun 状态并触发一次 checkpoint 持久化。
