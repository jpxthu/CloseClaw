...
Agent 未完成验证时可继续执行。Engine 待下次验收判定条件满足时重新注入 verify 消息。每次注入 pending_verify 计数加一，达到上限（默认 3 次，可在 workflow 定义中配置）则 phase 转为 blocked 并通知 Owner。Owner 回复后 Engine 解除阻塞，保留当前步骤目标消息，pending_verify 归零，清理残留 verify 消息，注入 verify。
...
正常结束（jump 结果为 complete）或 Owner 终止后，Engine 从追加区移除 workflow context，清理消息历史中的 workflow 控制消息（goal + recovered 恢复提示），清空 WorkflowRun 状态，主动触发一次 checkpoint 持久化空状态，session 恢复为普通 session。
...
- **Skills**：workflow 定义与技能隔离同维度（Agent 专属 > 全局 > 系统内置），但不走 system prompt skill listing。
