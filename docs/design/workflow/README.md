...
2. **Verify**：Agent turn 结束、session idle 时 Engine 注入验收清单，Agent 自查——未完成则继续执行，完成则调用 workflow_verify()
3. **Jump**：Engine 注入跳转问题，Agent 回答跳转条件，Engine 匹配 transitions 决定 goto/reexecute/complete

Engine 抹除 verify/jump 交互记录，更新 WorkflowRun 状态，注入下一步 goal。循环直到 complete。
