...
2. Engine 按优先级查找定义文件（命中即用，不继续查找下一级）：
   1. agent workspace 下的 workflows/ 目录
   2. .closeclaw/ 下的 workflows/ 目录
   3. 内置
3. Engine 解析 YAML frontmatter
4. Engine 初始化 WorkflowRun：current_step = 0，phase = executing
5. Engine 向 system prompt 注入 workflow context
6. Engine 注入 Step 0 goal 消息（role: workflow）
7. 返回 tool result 确认

### workflow_verify

1. Agent 调用 workflow_verify()（无参数）
2. Engine 检查当前 phase：
   - ≠ verifying → 返回错误
   - = verifying → phase 转为 jumping，注入 jump 消息
3. 返回 tool result（被抹除）
...
答案格式由 jump 问题定义中的 type 决定：

- boolean：true / false
- enum："A" / "B" / "C" / "D"（对应选项序号，非 option 内部值）
- string：自由文本
