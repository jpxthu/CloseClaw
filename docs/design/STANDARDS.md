...
- **字段列表、键值对 → 用无序列表**，每行一条，格式 `- key = value` 或 `- key：说明`
- **多属性对比 → 用表格**
- **层级关系、流程步骤 → 用 ASCII 图或嵌套列表**
- **禁止行内平铺**：`field1=xxx、field2=yyy、field3=zzz` 这类逗号/顿号拼接的长句一律改写为列表
...
| 类别 | 定义 | 放 common 文档 |
|------|------|---------------|
| 骨架类型 | 真正的跨领域概念，无单一领域归属 | ✅ 完整定义在 common |
| DI trait | 领域注入接口，概念上归属具体领域但在 common 供多方消费 | ✅ 在 common 索引，标注归属领域 |
| 单消费类型 | 仅被一个模块消费的配置/错误/内部类型 | ❌ 不入 common，保留在领域模块文档 |

**判断原则**：一个类型被哪个模块定义和理解。如果只有 gateway 模块理解 `HandleResult` 的含义，它就属于 gateway 文档——不应为了"共享"而入 common。

此标准同时约束代码层：文档中判定为单消费类型的，代码中也应移回领域 crate。
