...
IM Adapter 模块提供跨 IM 平台的插件化适配框架。每个 IM 平台封装为一个独立插件，包含协议适配（Adapter）和格式渲染（Renderer）两部分。Gateway 按平台选择对应插件，不关心插件内部实现。
...
- **插件接口层**：定义 IMPlugin trait，统一插件契约。每个 IM 平台实现此 trait，提供 platform 标识、入站消息解析、出站消息发送和格式渲染四项能力。
- **通用渲染能力**：代码块语法高亮和流式增量渲染是跨平台通用机制，由模块顶层提供，各平台插件直接复用。
- **平台插件**：每个 IM 平台一个文件，放在 `platforms/` 子目录下。插件内部包含——Adapter（webhook 解析、token 管理、API 调用）和 Renderer（ContentBlock[] → 平台原生格式）。
...
| `account_id` | 租户/账号标识，可选。用于多租户 session 隔离 |
| `content` | 消息文本内容 |
| `timestamp` | 消息发送时间 |
...
[IMPlugin 渲染]
  ...
  ↓
[IMPlugin 发送]
  Adapter.send(payload, peer_id, thread_id) → IM 平台
```
...
- **下游**：无——IMPlugin 是链路终点，负责与外部 IM 平台通信
