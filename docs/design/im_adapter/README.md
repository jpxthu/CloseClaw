...
**身份映射**：`account_id` 由 IM 插件在解析入站消息时填入。映射规则：以 sender_id 为键查询 `config/accounts.json` 中的账户绑定表，找到对应的 CloseClaw 账户 ID。一个账户可绑定多个平台的 sender_id（如飞书 open_id + Telegram user_id 映射为同一个 account_id）。terminal 渠道恒为 "owner"，无需查表。详见 [config 模块 accounts.json](../config/README.md)。
