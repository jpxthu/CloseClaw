...
- **插件注册机制**：编译期自动扫描 `platforms/` 目录发现所有平台插件并注册。不在 `platforms/` 下的插件（如 CLI 模块的 terminal）通过显式注册加入 Plugin Registry。启动时仅加载配置文件中显式启用的平台。各已启用平台的初始化相互独立——单个插件初始化失败仅记录日志并跳过该平台，不影响其他已启用平台的正常加载与运行。新增 IM 平台 = 新增目录 + 实现 trait，Gateway 代码无需改动。新平台默认禁用，需在配置文件中显式添加并启用方可使用。
