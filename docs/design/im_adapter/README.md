...
- **离线对照**：单元测试与解析逻辑以已采集的真实事件样本（fixture）为权威输入，样本存于 `tests/fixtures/<平台>/`；平台接口行为与限制（如媒体大小上限、格式约束）另以平台官方 API 文档为权威来源，开发实现与单元测试须对照两者验证（见各平台插件文档，如 [feishu 插件「媒体平台约束」](platforms/feishu.md)）
