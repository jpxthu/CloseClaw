...
流式渲染是 IM Adapter 模块的通用渲染子功能，负责在 LLM 流式输出时逐条渲染 ContentBlock 增量（定义见 [common ContentBlock](../common/shared-types.md#contentblock)）。用户无需等待完整响应即可看到输出内容。该能力以通用组件形式提供——各平台插件组合持有流式渲染器实例，在渲染时委托调用，平台可按需覆盖实现差异化渲染逻辑。

## 架构

流式渲染器由三个内部组件组成，均在 Gateway 的增量阶段被调用——ContentBlock[]（经 VerbosityFilter 过滤 + DslParser 透传）到达后进入流式渲染，完整链处理（DslParser 解析 DSL 指令 + 出站日志）由 Gateway 在收尾阶段调度执行：

- 行缓冲：对 Text 块内容逐行积累，以句末标点或换行为边界切分输出单元
- 类型路由：按块类型选择渲染路径
- 增量输出：完整输出单元立即通过 IMPlugin 发送

流式渲染器接收增量阶段的 ContentBlock[]，以增量方式渲染——Text 块内容逐缓冲行输出，Thinking/Tool 块等待完整内容后一次交付平台格式渲染器。交互式 UI 元素（按钮、选择器等）通过工具调用结果由 Gateway 直接处理，不属于流式渲染器职责范围。
...
- Thinking/Tool 块不参与流式行缓冲，累积完整内容后一次交付平台格式渲染器；Image/Audio/File 块不参与流式渲染，直接交由平台格式渲染器处理
- 代码/文本模式状态：检测 ``` 边界标记切换，用于决定行边界判定规则（代码模式按换行，文本模式按句末标点）

## 数据流

1. ContentBlock[] 增量到达 Gateway（来自 Session 的 LLM 流式响应）
2. Gateway 的 Processor Chain 增量阶段依次执行：VerbosityFilter 过滤 → DslParser 透传（零开销）→ 跳过 OutboundRawLog（出站调试日志，仅在 raw_log_dir 配置时注册）
3. Gateway 交付 ContentBlock[] 给 IMPlugin 流式渲染器
4. 流式渲染器按 ContentBlock 类型逐块处理：
