...
1. **Phase 0：信号接收 & 模式判定**
   - SIGTERM（首次）→ Graceful 模式，发送关闭启动通知告知 Owner 系统正在关闭（drain 结束后将展示 session 进度详情）
...
用户可选择等待或升级为 forceful。关闭启动时（Phase 0 模式判定后）立即发送首次通知，告知 Owner 系统正在关闭。进入 Session 停止阶段后切换为进度详情通知，有状态变化时更新（session 完成、新 session 开始停止等）。

会话停止期间若长时间无状态变化，系统每 30 秒发送一次心跳通知，让 Owner 确认系统仍在关闭中而非卡死。心跳内容为简化格式，不逐条列出 session 详情：

```
⏳ 仍在关闭中，3 个 session 活跃（最长等待 27s）
