...
Daemon 启动（依赖驱动，按拓扑序分层执行）
  │
  ├── 层 1（无依赖，并行初始化）
  │   ├── ConfigManager（多文件合并、凭据分离、.env 加载、openclaw.json 迁移）
  │   └── Storage（SqliteStorage 初始化）
  │
  ├── 层 2（依赖层 1，并行初始化）
  ...
  │   ├── Skills Registry（扫描技能目录、加载 builtin skills、合成注册表）
  ...
  │   ├── Tools Registry（各模块 register_tools() 注册工具定义，SpawnController 注入）
  ...
  │   ├── System Prompt 构建器（SessionManager 内部调用构建函数，持有 PromptOverrides，初始为 None）
...
| 模块 | 关系 |
|------|------|
| Config | 启动时加载各配置文件，合并为各组件所需的数据结构 |
...
| Skills Registry | 启动时创建注册表，扫描技能目录并填充 |
| Session Manager | 启动时创建并注入依赖，Daemon 持有其所有权 |
| System Prompt | SessionManager 内部调用系统 prompt 构建函数组装，持有 PromptOverrides（初始 None） |
