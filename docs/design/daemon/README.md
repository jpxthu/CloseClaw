...
Daemon 启动
  ├── 阶段一：基础设施
  │     Config 加载 → Storage 初始化 → Permission Engine → Agent Config 扫描
  ├── 阶段二：能力注册
...
Daemon 启动
  ...
  ├── 3. Permission Engine 初始化（加载规则 + 默认策略）
  ├── 4. Agent Config 扫描（三级扫描 + 字段合并 → 注册表）
  ├── 5. Tools Registry + Skills Registry（注册所有工具和 skill）
  ├── 6. Session Manager 创建（注入 storage、agent config、tool/skill registry）
  ├── 7. System Prompt Builder 创建（由 Session Manager 持有引用）
  ├── 8. Processor Registry（注册入站/出站处理器，按 priority 排序）
  ├── 9. Renderer Set（各平台 Renderer 注册）
  ├── 10. IM Adapters（各平台 Adapter 创建，注入对应 Renderer）
  ├── 11. Gateway 创建（注入 adapters、processor registry、renderers、session manager）
  ├── 12. ArchiveSweeper spawn（独立后台任务，定时扫描 idle session 并归档）
  ├── 13. Skill Watcher spawn（独立后台任务，监听 skill 文件变更）
  ├── 14. ChatServer 启动（TCP 服务器，通过 Gateway 路由消息）
  └── 进入消息循环
```
...
| # | 验证方向 | 进度 | 验证内容 |
|---|---------|------|---------|
| 1 | 初始化顺序 ←→ 依赖关系 | ⬜ | 初始化顺序是否符合各模块设计文档声明的依赖关系 |
| 2 | 后台任务 ←→ Session | ⬜ | ArchiveSweeper 机制与 session/README.md 一致 |
