...
链处理完毕后产出 [ProcessedMessage](../common/shared-types/processed-message.md)，由 Gateway 协调 IM Adapter 完成渲染和发送。流式和非流式出站统一经此链，VerbosityFilter 和 DslParser 对流式增量文本零开销透传。
...
```
Gateway 从 Session 读取 ContentBlock[]，构造 [ProcessedMessage](../common/shared-types/processed-message.md)
  ↓
...
Gateway 选择目标平台 IM 插件
  → 接收 ContentBlock[]（定义见 [common ContentBlock](../common/shared-types/content-block.md)）+ DslParseResult（定义见 [common DslParseResult](../common/shared-types/dsl-parse-result.md)）
  → 按块类型选择渲染策略
  → 输出平台原生格式并发送
...
LLM 输出 UnifiedResponse（含 ContentBlock[]）
  → Session 写入 messages[]
    → Gateway 从 Session 读取 ContentBlock[]
      → 构造 [ProcessedMessage](../common/shared-types/processed-message.md)，启动出站 Processor 链
        ...
        → OutboundRawLog（pri 20）：写入出站日志
            ↓
      → 链输出 [ProcessedMessage](../common/shared-types/processed-message.md)
        → Gateway 选择目标平台 IM 插件
...
- **下游**：[IM Adapter](../im_adapter/README.md) 模块（消费 ContentBlock[] + DslParseResult（定义见 [common DslParseResult](../common/shared-types/dsl-parse-result.md)），渲染为平台格式并发送）
- **链内**：
