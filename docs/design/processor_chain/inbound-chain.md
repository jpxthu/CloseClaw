...
```
IM Adapter 产出 NormalizedMessage { platform, sender_id, peer_id, content, ... }
  → RawLogProcessor：记录原始内容到日志 → 透传
    → SessionRouter：计算 session key = f(platform, sender_id, peer_id) → 查找/创建 session → session_id 写入 metadata
...
- **链内**：
  ...
  - SessionRouter — session 路由，使用 (platform, sender_id, peer_id) 三元组
  - MessageCleaner — 内容清洗
