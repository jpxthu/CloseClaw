...
IM 平台消息到达
  ↓
IM Adapter（入站）— 平台特定解析，产 NormalizedMessage（媒体已落盘为引用）
  ↓
...
3. ContentNormalizer（priority 30）
   ...
   → 非 text 消息（image/file/audio/post）跳过标准化，直接透传
  ↓
ProcessedMessage
  ↓
Gateway 路由
...
- `reply_ref` 不参与——出站定向引用，经 Session 上下文存储后供出站定向投递
...
其余消息（含非 text 类型）正常产 NormalizedMessage（标记 message_type，media_refs 承载已落盘引用，纯媒体消息 content 可为空），不做丢弃处理，由 Gateway 分型路由处理（媒体形态规则见 [im_adapter media-store](../im_adapter/media-store.md)）。
...
4. **ContentNormalizer**：文本标准化（去控制字符、压缩空行、去尾空格）。非 text 消息（由 message_type 判断）跳过标准化，直接透传 content
