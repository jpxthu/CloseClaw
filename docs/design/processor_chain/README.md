...
Processor 链（入站，按 priority 顺序执行，纯变换）
  ...
  ↓
Gateway
  → 提取稳定路由键 → SessionManager.resolve(稳定路由键) 获得 session_id
...
Renderer 层（传递格式 → 展示格式的唯一转换点，各渠道模块提供自身实现）
  ...
  ↓
IM Adapter（出站，IM Adapter 模块含 Renderer + Adapter）→ Renderer 完成格式转换，Adapter 根据 (peer_id, thread_id) 发送到对应会话/话题
```

关键设计：
- **入站链纯变换**：只做内容计算和 metadata 填充，不管理 session 生命周期、不做路由决策。Session 创建和查找由 Gateway 调用 SessionManager 负责
- **出站方向 ContentBlock[] 作为传递格式**（完整变体定义见 [common ContentBlock](../common/shared-types/content-block.md)）贯穿 Processor 链和 Renderer，不在链中途转为展示格式
...
- **入站归一化**产 NormalizedMessage（完整字段定义见 [common 共享类型](../common/shared-types/README.md)），经 Processor 链清洗和 metadata 填充后交付 Gateway，Gateway 做路由决策后进入 Session
...
NormalizedMessage 定义见 [common 共享类型](../common/shared-types/README.md)。IM Adapter 的入站部分负责将自己平台的格式转为此结构。内容块（ContentBlock[]）概念仅在出站方向使用——LLM 输出 UnifiedResponse 时引入，经出站链处理后由 Renderer 渲染。

Processor 链在入站方向按 priority 升序执行。链中处理器可操作 NormalizedMessage 的 content 字段（如 ContentNormalizer 做文本标准化——去除控制字符和 ANSI 转义序列、压缩连续空行、去行尾空格），也可向 metadata 写入计算结果（如 SessionRouter 写入 session_key）。平台格式转换和清洗由 IM Adapter 在解析阶段完成。链输出 [ProcessedMessage](../common/shared-types/processed-message.md)，由 Gateway 消费。
...
Processor 链（VerbosityFilter → DslParser → OutboundRawLog）
    ↓
[ProcessedMessage](../common/shared-types/processed-message.md)（DSL 结果写入 metadata["dsl_result"]，ContentBlock[] 透传）
    ↓
...
- **无关**：无（Processor Chain 的消息流经链路明确，无容易混淆的无关模块）
