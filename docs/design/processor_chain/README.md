...
processor_chain 模块管理入站消息归一化和批量出站消息的 DSL 解析。入站方向将各 IM 平台的原始消息归一化为统一结构，出站方向解析 LLM 批量输出中的 DSL 指令。流式出站不经 Processor 链——流式增量文本无 DSL 指令，直接走渲染路径。渲染和出站日志由 Gateway 层处理，不属于 Processor 链。

核心职责：
- 入站：接收各平台原始消息 → 归一化为统一的中间格式 → 清洗后交付上层
- 出站：接收 LLM 的批量结构化输出 → 解析 DSL 指令 → 交付渲染
...
- **上游**：Gateway（调度链执行）、Session（LLM 对话产出的 ContentBlock[]，经 Gateway 传递进入链，属数据流上游依赖）、IM Adapter（入站方向：产出 NormalizedMessage 供链消费）
- **下游**：[IM Adapter](../im_adapter/README.md) 模块（消费链输出并渲染为平台格式，发送渲染后的消息）
- **无关**：无（Processor Chain 的消息流经链路明确，无容易混淆的无关模块）
- **间接相关**：Slash Command 模块（斜杠指令经入站链计算 session_key + 内容清洗后由 Gateway 路由到 SlashDispatcher；SlashDispatcher 的输出经出站链 DSL 解析 + 日志后由 Renderer 回复。数据流双向穿越 Processor 链，但非直接调用依赖）
