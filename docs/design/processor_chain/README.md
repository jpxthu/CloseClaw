...
核心职责：
- 入站：接收 IM Adapter 产出的 NormalizedMessage → 内容清洗 → metadata 填充 → 交付上层
...
- **入站归一化**产 NormalizedMessage（完整字段定义见 [IM Adapter 模块](../im_adapter/README.md)），经 Processor 链清洗和 metadata 填充后交付 Gateway，Gateway 做路由决策后进入 Session
