...
- **Renderer 不在 Processor 链内**（详见 [renderer 模块](../renderer/README.md)），渲染是终结操作，需要路由信息（msg_type），不适合链的"变换传递"语义
- **入站归一化**产 NormalizedMessage（platform, sender_id, peer_id, content），经 Processor 链清洗后交付 Session/LLM
...
- **下游**：[renderer](../renderer/README.md) 模块（消费链输出并渲染为平台格式）、IM Adapter（发送渲染后的消息）
