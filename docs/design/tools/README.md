...
各模块通过 [ToolRegistrar](../common/core-traits.md#toolregistrar) trait 向 [ToolRegistry](../common/core-traits.md#toolregistry) 注册工具。Tools 模块收集所有 ToolRegistrar 实现者，按优先级编排调用，提供 [ToolRegistry](../common/core-traits.md#toolregistry) 的具体实现，不硬编码各模块的具体接口。ToolRegistry 自身不维护工具清单，清单由各模块的设计文档分别描述。工具的定义和参数归所属模块，tools 模块负责编排注册顺序。

### 索引结构

一级索引按分组输出：常用工具展示名称、危险度标记和行为描述，延迟工具仅展示名称和危险度标记。分组格式、危险度标记取值、加载策略标注与长度截断规则详见 [tools-prompt-injection.md](tools-prompt-injection.md)。
...
工具的权限检查不在单个工具实现中处理——工具执行前由 tools 模块的编排层统一调用权限引擎校验。工具本身通过运行时标记声明自身的安全属性（只读/破坏性/昂贵），供权限引擎和索引渲染使用。
...
```
系统启动
  → tools 模块集中编排，依次调用各模块的 register_tools(registry)
    → 各模块向 ToolRegistry 注册自身工具
...
| System Prompt 静态层 | ToolRegistry 的分组索引数据通过 [ToolsFragmentProvider](../system_prompt/fragment-provider.md)（实现 [PromptFragmentProvider](../common/core-traits.md#promptfragmentprovider) trait）注入 system prompt 静态层 ToolsSection |
