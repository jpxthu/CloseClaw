...
| [tool-registrar.md](tool-registrar.md) | ToolRegistrar trait：各模块向 ToolRegistry 注册工具的统一接口契约 |
...
各模块通过 [ToolRegistrar](tool-registrar.md) trait 向 ToolRegistry 注册工具。Tools 模块收集所有 ToolRegistrar 实现者，按优先级编排调用，不硬编码各模块的具体接口。ToolRegistry 自身不维护工具清单，清单由各模块的设计文档分别描述。工具的定义和参数归所属模块，tools 模块负责编排注册顺序。
