...
各模块暴露 `register_tools()` 方法，tools 模块在启动时集中编排调用——依次调用各模块的 `register_tools(registry)` 完成注册。ToolRegistry 自身不维护工具清单，清单由各模块的设计文档分别描述。工具的定义和参数归所属模块，tools 模块负责编排注册顺序。
