...
各模块暴露 `register_tools()` 方法，tools 模块在启动时集中编排调用。ToolRegistry 自身不维护工具清单，清单由各模块的设计文档分别描述。工具的定义和参数归所属模块，tools 模块负责编排注册顺序。

### 注册编排

启动时，tools 模块按以下顺序调用各模块的 `register_tools(registry)`：

1. tools 核心工具（bash / file_ops / git_ops / meta）
2. Session 模块（sessions_spawn / sessions_steer / sessions_kill）
3. Skills 模块（SkillTool / SkillCreator）
4. IM Adapter 模块（飞书平台工具分组）
