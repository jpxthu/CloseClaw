...
| [session-compaction.md](session-compaction.md) | 会话上下文压缩：触发机制、LLM summarization、bootstrap 保护（防角色/规则在压缩中丢失） |
| [llm-session-enhancements.md](llm-session-enhancements.md) | LLM 交互增强：流式输出、Reasoning Level 控制、Cache Hit 统计、Thinking 标签处理 |
...
- **压缩**在 session 运行时发生：对过长的对话历史做 summarization，bootstrap 保护机制确保压缩不丢失核心规则。
