...
- **持久化层组件**：
  - **SessionManager**：session 的创建、查找、恢复入口。维护 session_key → session_id 映射表。查询时：key 命中返回已有 session，未命中创建新 session 并写入映射。`/new` 指令创建新 session 后覆盖映射。协调各组件完成 session 初始化。session_id 格式为 `{agent_id}_{timestamp}_{random_suffix}`。
  ...
  **key_registry 生命周期**：
  ...
  - 运行时：查映射表获取已有 session；创建新 session 后覆盖映射
...
用户消息到达 Gateway
  ...
  → SessionManager 查找或创建 session
    ...
    → 命中 → 检查 session 持久化状态
      → active → 返回已有 session
      → archived → 触发恢复 → Gateway 通知用户 → 返回恢复后的 session
    → 未命中 → 创建新 session → 注册到映射表
...
```
再次访问已归档 session
  → SessionManager 发现状态为 archived
  → 通知用户 "正在恢复会话..."
  → 恢复 session 持久化数据
    → transcript 移回活跃区
    → session 状态更新为 active
