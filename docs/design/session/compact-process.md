...
1. 压缩前由 Run Health 模块创建可回滚的 transcript 快照（详见 run-health.md）
2. 从消息历史中分离对话消息（排除 system prompt）
3. LLM 对对话消息做结构化摘要
4. 组装压缩后 transcript：boundary 消息
5. 通知 SessionManager 触发 system prompt 静态层重建（详见 session-injection.md）
6. 持久化 SessionCheckpoint（transcript 仅包含 boundary 消息，不含 system prompt）
...
**分级告警阈值**：根据剩余 token 空间分为正常、预警、触发自动压缩三个级别。

**熔断器**：连续自动压缩失败达到上限后，自动压缩暂停，避免反复失败消耗资源。手动 `/compact` 不受熔断器限制——手动压缩总是允许执行，手动压缩的失败不递增熔断计数器。手动压缩成功后熔断器自动复位。熔断触发时，系统注入一条 assistant 消息到 transcript 提示用户「自动压缩已暂停，建议手动 /compact」。熔断暂停仅阻止自动压缩再次触发，不影响活跃判定（inactive 判定与归档流程正常执行）。
...
**boundary 消息**是压缩后的 transcript 中的唯一消息条目，承载 LLM 生成的对话历史结构化摘要（覆盖六个维度）及压缩元信息（压缩时间、触发方式）。压缩完成后 transcript 的对话部分只包含这一条 boundary 消息，替代压缩前的全部 user/assistant 消息。

压缩 prompt 要求 LLM 覆盖六个维度：Goal（目标）、Constraints & Preferences（约束与偏好）、Progress（进度：Done/In Progress/Blocked）、Key Decisions（关键决策）、Next Steps（下一步行动）、Critical Context（关键上下文）。
...
4. 分支判断：
   - 若处于预警阶段且未提示过：提示用户「对话即将压缩，可输入 /compact 手动管理」，标记已提示（同一轮阈值区间内不重复提示）
   - 若触发自动压缩：
...
不触发时：跳过压缩，继续正常对话流程。自动压缩失败时：跳过压缩，熔断计数器递增，继续正常对话流程。熔断器首次触发（计数达到上限）时：注入一条 assistant 消息到 transcript 告知用户「自动压缩已暂停，建议手动 /compact」。后续检查时若熔断器仍处于触发状态，不再重复注入。继续正常对话流程。
