...
**熔断器**：连续自动压缩失败达到上限后，自动压缩暂停，避免反复失败消耗资源。手动 `/compact` 不受熔断器限制——手动压缩总是允许执行，手动压缩的失败不递增熔断计数器。手动压缩成功后熔断器自动复位。熔断触发时，系统注入一条 assistant 消息到 transcript 提示用户「自动压缩已暂停，建议手动 /compact」。
