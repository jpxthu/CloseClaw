...
TerminalAdapter 从 stdin 读取用户输入，封装为 NormalizedMessage（字段定义见 [common 共享类型](../common/shared-types/README.md)）。terminal 渠道的字段取值：
...
TerminalRenderer 接收 ContentBlock[]（定义见 [common ContentBlock](../common/shared-types/content-block.md)）和 DSL 解析结果，转换为 ANSI 格式的 RenderedOutput。TerminalPlugin 通过 send 方法将 RenderedOutput 写入 stdout。渲染与发送分离，遵循 IM Adapter 框架的设计原则。详细渲染策略见 [Terminal Renderer](renderer.md)。
