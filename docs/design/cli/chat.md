...
用户通过 `--agent-id` 指定目标 agent。agent_id 随消息传递，SessionRouter 将其纳入 session key 计算——同一用户对不同 agent 维持独立的 session。
...
空行检测 → 空内容跳过
  ↓
TerminalAdapter 封装 NormalizedMessage { platform: "terminal", sender_id, peer_id: "cli", agent_id, content, timestamp }
  ↓
Processor Chain 入站
  ├── RawLog：记录原始输入到日志
  ├── SessionRouter：根据渠道、用户、端点和 agent 计算 session key
  ├── MessageCleaner：清洗 stdin 残留控制字符
  └── MarkdownNormalizer：标准化格式
  ↓
