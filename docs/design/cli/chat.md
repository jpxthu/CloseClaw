...
> **流式路径**：LLM 流式输出时，不走上述经 Processor Chain 的批量渲染路径。Gateway 通过 IMPlugin trait 的流式默认方法驱动 DefaultStreamingRenderer，直接逐行产生增量 RenderedOutput 后由 TerminalPlugin 发送到 stdout。流式文本输出不经 Processor Chain——文本增量无需 DSL 解析器参与。流式结束后 Gateway 记录出站日志。详见 [IM Adapter 流式渲染](../im_adapter/streaming-render.md)。
