...
- 配置内容由 Agent 模块或 Config 模块在系统启动时加载并缓存
...
4. Gateway 根据 SlashResult 变体构造 SideEffectContext → 触发副作用执行（回复、切换模式、创建 session 等）→ 副作用产出 ContentBlock[] 进入出站 Processor Chain
...
1. 系统启动 → System Prompt Builder 实现 SystemPromptBuilder → 收集所有 PromptFragmentProvider 实现者并注册
2. 触发机制与 PromptFragmentProvider 流程整合（session 创建 / 恢复 / compaction）
3. SystemPromptBuilder 构建 FragmentContext → 按优先级遍历 Provider → 检查缓存（命中复用，未命中调用片段生成）
4. 拼接所有非空片段 → 返回完整的 system prompt 字符串 → 写入 Session 的 system prompt 字段
