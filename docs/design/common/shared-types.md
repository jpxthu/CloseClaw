...
1. 用户消息以 / 开头 → Gateway 路由到 SlashDispatcher
2. SlashDispatcher 解析指令名和参数 → HandlerRegistry 查表
3. Handler 处理，返回 SlashResult 变体（Handler 的工作到此结束）
4. Gateway 构造 SideEffectContext（携带 session 操作能力和回复通道）
5. Gateway 触发 SlashResult 变体执行，各变体自行完成副作用：
   - 回复内容 → ContentBlock[] → 出站 Processor Chain → IM Adapter 渲染发送
   - Session 操作（模式切换、创建、停止、压缩等）→ 通过 SideEffectContext 操作当前 session
