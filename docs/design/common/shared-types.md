...
```
用户消息以 / 开头 → Gateway 路由到 SlashDispatcher
  ↓
SlashDispatcher 解析指令名 + 参数 → HandlerRegistry 查表
  ↓
Handler 处理 → 返回 SlashResult 变体
  ↓
Gateway 构造 SideEffectContext（session 操作能力 + 回复通道）
  ↓
SlashResult 变体执行，同时触发两类操作：
