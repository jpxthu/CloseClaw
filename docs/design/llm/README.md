...
LLM Client（UnifiedChatClient）—— 统一入口
  │
  ├─ CacheAdapter —— 请求前置处理，标记静态区缓存控制参数
  │    └ 仅请求路径
  │
  ├─ Plugin Pipeline —— 模型专属行为注入（per-model 绑定）
  │    请求：before_request（注入模型特定参数）
  │    响应：after_response（后处理内容块）
  │    流式：on_stream_event（逐事件过滤/加工）
  │
  ├─ ModelInterpreter —— 字段归一化
  │    └ 仅响应路径（协议原生字段 → 统一内容块 + 流式事件）
  │
  ├─ Protocol —— 协议适配（OpenAI / Anthropic）
  │    请求：序列化
  │    响应：解析 JSON + 流式 SSE 状态机
  │
  └─ Provider —— HTTP 基础设施
       base_url、api_key、HTTP 发送、接收、厂商错误映射
```
...
- **共享类型 / 核心 trait**：[common/core-traits](../common/core-traits.md)（消费：ShutdownSignal）
