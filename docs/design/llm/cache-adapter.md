...
缓存适配器依赖 system prompt 模块的 `STATIC_LAYER_END` 边界标记来切分可缓存前缀和不可缓存后缀。标记是 cache adapter 的程序输入，而非仅供人类阅读的装饰文本。标记之前的内容作为静态区，标记之后的内容作为动态区；cache adapter 只对静态区标记缓存控制参数，动态区内容透传不做修改。

### 消息历史缓存

支持显式前缀缓存的供应商（Anthropic）在 messages 数组尾部标记 cache_control，使下一轮请求的完整前缀（上轮的 system prompt + 全部 messages）命中缓存。每轮请求只在消息序列的最尾部打一个 cache_control，指向前缀匹配点。
