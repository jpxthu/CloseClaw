...
| `communication` | 跨 agent 交互权限 | 否 | 见子字段 |
...
`communication` 控制本 agent 在运行中与其他 agent 的交互权限。spawn 前的准入由 `subagents.allowAgents` 控制，spawn 完成后的运行时交互由 `communication` 控制。

| 字段 | 含义 | 默认值 |
|------|------|--------|
| `allowAnnounceFrom` | 允许哪些 agent 向本 agent 推送 announce（子 session 完成通知） | `["*"]` |
| `allowSteerFrom` | 允许哪些 agent 对本 agent 的 session 执行 steer 操作 | `[]` |
| `allowKillFrom` | 允许哪些 agent 对本 agent 的 session 执行 kill 操作 | `[]` |

三个字段均支持 `["*"]`（不限制）和 `[]`（禁止所有）。默认值含义：默认允许所有 agent 推送 announce，但不允许任何 agent 主动 steer 或 kill——这些操作需要显式授权。

`communication` 与 `subagents` 同为字段级覆盖合并，支持全局和 per-agent 独立配置。
...
| 通信能力 | `communication.allowAnnounceFrom` / `allowSteerFrom` / `allowKillFrom` | 控制跨 agent 运行时交互 |
