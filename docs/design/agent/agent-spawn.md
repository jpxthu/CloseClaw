...
Spawn 是父 session 创建子 session 执行子任务的机制。一次 spawn = 创建一个 child session，子 session 使用目标 agent 的配置档案运行。Spawn 由 tools 模块暴露给 LLM 调用，由 Session 模块负责协调控制——Session 模块读取父 agent 配置中的 subagents 参数执行前置检查，并创建和管理子 session。
...
- announce 通过优先级消息队列注入父 session 对话流
...
系统在执行前检查目标 agent 的 `communication` 配置（详见 agent-config.md）：
- Steer → 检查 `allowSteerFrom`
- Kill → 检查 `allowKillFrom`
发起方 agent ID 不在白名单中则拒绝操作。
...
| 模块 | 调用关系 |
|------|---------|
| Tools | 注册 sessions_spawn / sessions_steer / sessions_kill 工具，LLM 调用后委托 Session 模块执行 |
