...
Agent 模块负责两件事：Agent 配置档案的定义与加载、权限基线的提供。Agent 的身份人格由 Bootstrap 文件定义，加载由 System Prompt 模块处理。
...
- **Agent 配置档案**：每个 agent 对应一个独立的配置目录（`agents/<id>/`），目录下存放 `config.json` 和 `permissions.json`。配置定义能力边界（模型、工具、workspace、spawn 控制），权限独立存储。存储支持项目级和用户级两级优先级，字段级覆盖合并。详见 [agent-config.md](agent-config.md)。
- **Agent 能力模型**：Agent 能力完全由配置字段组合决定，不依赖预定义的类型枚举。初始 Agent 由 CLI 配置向导在首次运行时创建（默认 ID `master`），其他 agent 由用户通过配置文件自定义。
...
```
父 session 调用 sessions_spawn 工具（由 Tools 模块暴露）
  ↓
Session 模块读取父 agent 配置中的 subagents 参数
  → 前置检查（depth/并发/白名单/requireAgentId）
  ↓ （全部通过）
...
| Session | session 创建时读取 agent 配置档案，分发各字段；spawn 时读取 subagents 控制参数 |
| Skills | 会话型 skill（需独立 session 的 skill）通过 sessions_spawn 创建子 session |
| Tools | sessions_spawn / sessions_steer / sessions_kill 工具注册在 tools 模块 |
...
| 模块 | 消费方式 |
|------|---------|
| Session | 读取 model、workspace、subagents |
