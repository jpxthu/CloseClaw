...
Gateway/Daemon 确定目标 agent ID
  ↓
AgentRegistry::get(agent_id) → ResolvedAgentConfig
  ↓
Session 模块分发各字段到对应模块
...
| Session | session 创建时读取 agent 配置档案并分发各字段；spawn 时读取 subagents 配置参数；注册 sessions_spawn 等工具到 ToolRegistry |
...
| 模块 | 消费方式 |
|------|---------|
| Session | 读取 model、workspace、subagents；按 tools/disallowedTools 过滤工具集，按 skills 过滤可用 skill 列表 |
| System Prompt | 读取 bootstrapMode、agentDir；bootstrap 文件（AGENTS.md 等）通过 [BootstrapFragmentProvider](../system_prompt/fragment-provider.md)（实现 [PromptFragmentProvider](../system_prompt/fragment-provider.md) trait）注入 system prompt 静态层 |
