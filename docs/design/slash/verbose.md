...
- `off`：仅展示最终回复（含作为交付物的媒体块），隐藏思考/工具调用/工具结果等全部中间产物
...
`/verbose normal` 指令处理流程：

1. 处理器返回设置展示等级结果（目标等级 Normal）
2. Gateway 将等级写入 Session
3. 回复用户已设置的展示等级
4. 下一条回复 → VerbosityFilter 按等级过滤：
   - 思考过程 → 不展示
   - 工具调用 → 展示
   - 工具结果 → 展示
   - 最终回复 → 展示
...
1. Agent 回复内容到达（含思考块、工具调用块、工具结果块、文本块）
2. 读取 Session 存储的展示等级，按等级过滤：
   - `full` → 不过滤，全部内容块通过
   - `normal` → 移除 Thinking 内容块，保留工具调用、工具结果和最终回复内容
...
- **上游**：Gateway 通过 Dispatcher 路由到 VerboseHandler 处理指令
