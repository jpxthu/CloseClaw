...
**TraceContext**：追踪上下文管理。为每条日志事件管理 span 标识分配——主消息链路事件分配根 span，子调用事件从父 span 派生新 span。每条消息的 trace_id 由 IM Adapter 在入站事件到达时生成（由平台标识、到达时间戳、随机数组合得出），写入消息 metadata 后随消息在各模块间流转。无入站消息的系统内部事件（定时任务、后台任务等）由触发模块自行生成独立 trace_id。当主处理链路中触发子调用（如 LLM 调用工具执行、spawn 子 Agent）时，TraceContext 从当前 span_id 派生子 span 标识，子调用日志通过 parent_span_id 关联回主链路。
...
2. 事件携带 trace_id（IM 消息由 IM Adapter 在接收时生成；非消息事件由触发模块生成）和 session_key（来自 Processor Chain 产出的 metadata）
...
trace_id 在入站事件到达即产生，贯穿整条消息链路。日志事件同时携带 trace_id 和 session_key——trace_id 用于端到端消息链路串联，session_key 用于日志追踪和 session 上下文关联。
