...
Session 运行期 prompt 定制通过 `/system` 指令操作 AppendSection（详见 [追加区](#追加区)）实现：

- `/system add <内容>`：向 AppendSection 追加文本
- `/system clear`：清空 AppendSection
- `/system list`：查看当前追加列表

`/system` 指令的完整行为定义见 [slash/system-append.md](../slash/system-append.md)。
