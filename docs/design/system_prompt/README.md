...
| Section | 内容 | 来源 |
|---------|------|------|
| RoleSection | Agent 角色定义、身份标识 | Bootstrap 文件（按模式加载，见下方加载模式表），另整合 IDENTITY+SOUL |
...
SkillListingSection 由 DiskSkillRegistry 的 `generate_listing(agent_id)` 方法生成，按 agent 过滤可见 skill。若 skill 列表为空，不添加此 Section。listing 内容作为 Section 进入 system prompt，而非 session transcript 中的独立消息。
...
| SessionState | 当前活动的后台任务状态（pending_tasks） | ConversationSession 运行时字段 |
| GitStatus | 从 workdir 路径派生的 git 分支和变更状态（非 git 仓库时不注入） | session/working-directory 模块 |

动态层内容的约束：同一 session 内多次 API 调用间，动态层前缀内容应尽可能不变，以确保 cache adapter 能复用前一轮的 KV cache。每轮递增的计数器（如 turn_count）不进入 system prompt，转而通过 API metadata 字段传递。须每轮更新的信息（如当前时间戳）放在缓存边界之后，不影响静态前缀的缓存命中。
...
- Compaction → 强制重建全部静态层（通过 `rebuild_system_prompt`），确保继续对话时角色定义、工具列表、skill 清单和长期记忆均为最新版本

### Session 级 Prompt 定制

System prompt 的配置级定制通过切换 Agent 实现——不同 Agent 有独立的 bootstrap 文件，对应不同的身份和能力群组。

Session 运行期 prompt 定制通过 `/system` 指令操作 AppendSection 实现：

- `/system add <内容>`：向 AppendSection 追加一段文本（多次 add 叠加，不覆盖）
- `/system clear`：清空 AppendSection
- `/system list`：查看当前追加列表

AppendSection 位于 system prompt 末尾、动态层之后、对话历史之前，持久化在会话状态中，不受上下文压缩影响。详细设计见 [slash/system-append.md](../slash/system-append.md)。
...
SessionManager 创建新 session
  ...
  DiskSkillRegistry 生成 skill 摘要清单
  读取 MEMORY.md（命中缓存则跳过）
  →
  组装静态层：RoleSection + ToolsSection + SkillListingSection + MemorySection
  →
  写入 ConversationSession.system_prompt（运行时字段，不进 SessionCheckpoint）
```
...
- **DiskSkillRegistry**：提供 SkillListingSection 的 `generate_listing(agent_id)` 方法，读取磁盘 skill 并合并内置 SkillRegistry 的 skill，按 agent 过滤并按优先级排序。

### 无关

- **LLM Provider**（无调用关系）：system prompt 构建完成后通过 ConversationSession 传递给 LLM provider，构建流程本身不调用 provider。
- **Compaction 模块**（间接关联）：compaction 完成后通过 `rebuild_system_prompt` 回调触发 system prompt 重建，确保 Section 内容与最新文件一致。system prompt 本身不参与对话消息的压缩逻辑。
