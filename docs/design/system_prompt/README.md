...
System Prompt 由多个 Section 组成，分为静态层和动态层，两层之间通过边界标记分隔。
...
| AppendSection | Owner 通过 `/system` 命令追加的临时指令，当次生效后自动清除。最大 500 字符，超出截断并返回提示 | `/system` 命令 |
| GitStatus | 从 workdir 路径派生的 git 分支和变更状态（非 git 仓库时不注入） | workdir 模块 |

AppendSection 在请求结束后自动清除，不进持久化。
...
优先级 prompt 命中后跳过 Section 渲染，仅追加 AppendSection。动态层的 ChannelContext、SessionState、GitStatus 在优先级 prompt 场景下均不注入。
...
API 请求到达
  ...
  获取 AppendSection（/system 命令设置的临时内容）
  →
  拼接：静态层 + 边界标记 + 动态层 + AppendSection
  →
  发送 LLM 请求
  →
  请求完成后清除 AppendSection
```
