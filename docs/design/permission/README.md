...
**User 身份**：每个 User 通过 IM 渠道加 IM User ID 的组合唯一绑定。一个 User 可以绑定多个 IM 渠道（如飞书 open_id + Telegram user_id），映射为同一个 CloseClaw User ID。User 的默认权限与 Agent 独立设置——User 默认无任何权限，包括收发消息也需显式授予。不区分用户组，所有权限以个体为单位。
...
```
                ...
                │     ├─ Owner → 经 SlashDispatcher → Handler → 跳过权限直接执行
