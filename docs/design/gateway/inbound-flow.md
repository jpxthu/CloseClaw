...
[Processor Chain 入站]
  RawLog(10)        → 日志记录 → 透传
    ↓
  SessionRouter(20) → session_key = {timestamp}-{hash}
...
- 计算：`session_key = {timestamp}-{hash}`。timestamp 为消息到达时间的毫秒级 Unix 时间戳，hash 由 `platform:sender_id:peer_id:account_id:timestamp_ms` 拼接后计算，确保并发消息因时间戳不同而产生不同 hash
