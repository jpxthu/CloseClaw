...
出站（LLM 响应或 SlashHandler 输出均以 ContentBlock[] 进入）：
  ...
  ContentBlock[] → [Processor Chain 出站: DslParser→RawLog]
                 → ProcessedMessage
                 → [Gateway: 选择 Renderer → render(ProcessedMessage)]
                 → RenderedOutput
...
- RenderedOutput：Gateway（通过 Renderer）产出，IM Adapter 消费
...
Gateway 收到 ContentBlock[] 后（来源：LLM 响应或 SlashHandler），调度 Outbound Processor Chain 处理，然后根据目标平台选择 Renderer 调用 `render(ProcessedMessage)`，产出 RenderedOutput，交给 IM Adapter 发送。
