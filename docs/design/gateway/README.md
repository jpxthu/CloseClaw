...
斜杠指令的回复同样经此出站路径——SlashResult 变体通过 SideEffectContext 的回复通道产出回复内容，由 Gateway 送入出站 Processor Chain 处理（DslParser），经出站日志记录后由 IM 插件渲染发送。这保证了斜杠指令回复与 LLM 回复使用统一的日志记录和 DSL 解析链路。
...
SlashResult.execute() 通过 SideEffectContext 的回复通道产出回复内容，Gateway 将回复送入出站 Processor Chain（DslParser）、记录出站日志后由 IM 插件渲染发送。详见 [Slash 模块](../slash/README.md)。
