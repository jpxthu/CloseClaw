...
出站（ContentBlock[] 来源：LLM 响应由 Session 产出，或斜杠指令回复由 SlashResult 变体产出。Gateway 按交付模式选择不同执行时序）：
  ...
  流式模式：
    ContentBlock[chunk₀] → [VerbosityFilter → DslParser 透传] → [IM Adapter 增量渲染 → 逐片发送]
                        …（多轮 chunk）
    ContentBlock[] 完整 → [DslParser 完整解析 → OutboundRawLog（条件组件）]（收尾阶段，无新渲染输出）
                        → [Gateway: 出站历史记录持久化到 session checkpoint]
...
- **非文本消息处理**：若消息的 message_type 非 text（image/file/audio），Gateway 直接构造"暂不支持该消息类型"的错误回复（ContentBlock[]），经简化出站路径发送。简化出站路径是 Gateway 层面的出站通道选择——错误回复为纯文本不含 DSL 指令且无需按 Session 过滤，因此跳过 VerbosityFilter/DslParser/中间件；若 `raw_log_dir` 已配置则执行 OutboundRawLog 写调试日志，然后渲染发送。流程到此结束。
...
- SideEffectContext：Gateway 构造的执行上下文，封装 Session 引用和回复通道。Gateway 构造后触发 [SlashResult](../common/shared-types.md#slashresult) 执行，各变体通过上下文完成副作用
