...
```
IM Adapters（入站）→ Gateway 入站处理
                         ├── 调度 Inbound Processor Chain
                         │     RawLog → SessionRouter → MessageCleaner → MarkdownNormalizer
                         └── 路由决策
...
IM 平台 webhook → IM Adapter(入站) → NormalizedMessage
  →
Gateway 入站处理
  ├── 调度 Inbound Processor Chain（按 priority 升序）
...
| # | 验证方向 | 进度 | 验证内容 |
|---|---------|------|---------|
| 1 | 入站路由 ←→ Processor Chain | ⬜ | Gateway 描述的入站流程与 processor_chain/README.md 一致 |
