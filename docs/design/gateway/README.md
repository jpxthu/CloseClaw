...
- **`/` 开头 → 斜杠指令**：分派给 SlashDispatcher。
  - Immediate 指令（如 `/stop`、`/status`、`/help` 等）→ 绕过消息队列立即执行。
  - 非 Immediate 指令 → Handler 处理，返回 SlashResult，Gateway 构造 SideEffectContext 调用 execute() 执行副作用。
...
SlashDispatcher 返回 SlashResult 后，Gateway 构造 SideEffectContext 并调用 SlashResult.execute()。SideEffectContext 封装 Gateway 持有的 Session 引用和回复通道，各 SlashResult 变体在 execute() 内自行完成对应的 session 操作和用户回复。Gateway 不穷举变体，副作用逻辑内聚在 slash 模块。详见 [Slash 模块](../slash/README.md)。
...
| SlashDispatcher | SlashResult 由 Gateway 调用 execute() 执行副作用并出站 |
