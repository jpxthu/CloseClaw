...
- **Session 解析**：Gateway 从 metadata 取出 session_key。若 session_key 为空（SessionRouter 计算失败），Gateway 回复"会话路由失败"。非空时调用 `SessionManager.resolve(session_key)` 获得 `session_id`。
...
Gateway 收到 ContentBlock[] 后（来源：LLM 响应，由 Session 产出；或 SlashHandler），经 Verbosity 过滤后调度 Outbound Processor Chain 处理得到 ProcessedMessage，然后根据目标平台选择 IM 插件，插件内部渲染后直接发送。
...
| SessionManager | 调用 SessionManager.resolve() 获取 session_id，编排 Session 的创建、查找、恢复。生命周期实现由 SessionManager 负责 |
