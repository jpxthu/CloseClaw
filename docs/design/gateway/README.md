...
```
入站：
  webhook → [IM Adapter: 平台格式解析] → NormalizedMessage
...
出站（LLM 响应或 SlashHandler 输出均以 ContentBlock[] 进入）：
  ...
  ContentBlock[] → [Processor Chain 出站: DslParser→RawLog]
                 ...
                 → [Gateway: 选择 Renderer → render(content_blocks, dsl_result)]
                 → RenderedOutput
                 → [IM Adapter: msg_type 分发 → 平台格式发送]
```
...
- RenderedOutput：Gateway 调用 Renderer.render() 产出，IM Adapter 消费；由 Gateway 提取 payload 并委托 IM Adapter 发送
...
Gateway 收到 ContentBlock[] 后（来源：LLM 响应或 SlashHandler），调度 Outbound Processor Chain 处理得到 ProcessedMessage，然后根据目标平台选择 Renderer，调用 `render(content_blocks, dsl_result)`（其中 `content_blocks` 和 `dsl_result` 从 ProcessedMessage 中提取），产出 RenderedOutput，交给 IM Adapter 发送。
...
| 2 | 出站发送 ←→ Renderer | ✅ | Gateway 描述的出站流程与 renderer/README.md 一致 — 2026-05-26 验证通过，修复 render() 签名后收敛 |
| 3 | 斜杠拦截 ←→ Slash | ✅ | 斜杠拦截和 SlashResult 处理与 slash/README.md 一致 — 2026-05-26 验证通过，补充 Unknown 类型 |
| 4 | Session 交互 ←→ Session | ✅ | Gateway 调用 SessionManager 的方式与 session/README.md 一致，补充 archive restore 流程 — 2026-05-26 验证通过 |
| 5 | Permission 调用 ←→ Permission | ✅ | Gateway 调用 Permission 的场景与 permission/README.md 一致 — 2026-05-26 验证通过，无矛盾，视图差异不影响一致性 |
