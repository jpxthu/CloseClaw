...
- **系统通知接口**：Gateway 提供通用系统通知发送接口，供 Session 等模块发送纯文本系统通知（如"⏳ 正在排队..."、"正在恢复会话..."）。系统通知是面向 User 的提示，走简化出站路径投递到 IM，不注入 LLM 上下文、不写入会话对话历史。通知内容与触发时机由调用方模块负责。系统通知与"注入对话历史、供 LLM 上下文处理"的系统级消息（如子 Session 超时/预警通知）属于不同机制——边界的完整定义见 [出站流程·系统通知接口](outbound-flow.md#系统通知接口)。
...
- **简化出站路径**：媒体不可得提示、系统通知、降级提示（含流式中断与批量发送失败）及一切面向 User 的纯文本提示——跳过完整链，仅经调试日志（配置时）→ 渲染 → 发送，不写出站历史。
