...
流式出站不经 Processor Chain——流式增量文本由 Gateway 驱动 DefaultStreamingRenderer
直接逐行输出，流结束时 Gateway 记录出站日志。
```
...
Gateway 收到 ContentBlock[] 后（来源：LLM 响应，由 Session 产出；或 SlashResult 变体产出），对于批量出站：经 Verbosity 过滤后调度出站 Processor Chain 处理得到 ProcessedMessage，然后根据目标平台选择 IM Adapter 渲染后直接发送。对于流式出站：不经 Processor Chain，由 Gateway 驱动 DefaultStreamingRenderer 直接逐行输出，流结束后记录出站日志。
...
SlashResult 的执行通过 SideEffectContext 的回复通道产出回复内容，Gateway 将回复送入出站 Processor Chain（DslParser）、记录出站日志后由 IM Adapter 渲染发送。详见 [Slash 模块](../slash/README.md)。
