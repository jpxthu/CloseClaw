...
定期消费 SQLite 中已 mining 的 event，通过 entity 级频次评分和跨 event lesson 浓缩，将高价值行为规则写入 MEMORY.md（路径通过 `storage.memory_md_path` 配置，默认 `memory/MEMORY.md`）。
...
> 路径均为相对路径，以 CloseClaw 数据根目录（`~/.closeclaw/`）为基准。

1. 输入：SQLite events 表（增量，mined=true）
2. **Light** — 程序化处理：增量读取、去重（与 MEMORY.md 已有规则做语义去重）、按 entity type 分块
3. **REM** — 程序化处理：在 Light 分块结果内做 entity 级聚类（通过 event_entities 关联表统计 entity 频次），跨 session 共享相同 entity 的 event 被自动关联
4. **Deep** — 程序化处理：entity 级多维加权评分（含 entity type weight），对每个 entity 的关联 event 组打分，三道门槛过滤
5. **教训浓缩** — LLM 处理：对通过 Deep 门槛的 entity 组，将其关联 event 的 lesson 浓缩为单条行为规则
6. 输出：MEMORY.md（可执行的行为规则，路径通过 `storage.memory_md_path` 配置，默认 `memory/MEMORY.md`）
7. **Dream Diary**（可选）— LLM 叙事摘要，写入独立日记文件（路径通过 `dreaming.diary.path` 配置），不参与记忆升格链路
...
- 触发条件：dreaming 完成后，由 `dreaming.diary.enabled` 控制（开启 dreaming 后默认打开）
- 内容：本轮新增/变更的 MEMORY.md 条目摘要，以连贯散文形式呈现
- 产出：写入 `dreaming.diary.path` 配置的目录（默认 `memory/diary/`），不参与记忆升格链路，仅供用户查阅
...
由 Daemon 层的 DreamingScheduler 定时任务驱动，按 cron 表达式（`dreaming.schedule`）调度执行。DreamingScheduler 的整体调度顺序（先 dreaming 后 mining）详见 README。

## 数据流

1. 输入：SQLite events 表（增量，mined=true，含 entity 关联）
2. Light：增量读取 → 去重 → 按 entity type 分块
3. REM：entity 级聚类（SQL JOIN event_entities）→ entity 频次统计（精确计数）→ 跨 agent 模式标记
4. Deep：entity 级多维评分（含 type weight 加权）→ 三道门槛过滤
5. 教训浓缩（LLM）：跨条目 lesson 浓缩 → 输出行为规则
6. 输出：MEMORY.md（可执行的行为规则，路径通过 `storage.memory_md_path` 配置）
7. Dream Diary（可选）：叙事摘要，写入 `dreaming.diary.path` 配置的目录
...
- **上游**：
  ...
  - LLM：教训浓缩和 Dream Diary 调用 LLM（模型通过 `dreaming.model` 配置）

- **下游**：
  - 文件系统：写入 MEMORY.md（`storage.memory_md_path`）和 Dream Diary（`dreaming.diary.path`）
