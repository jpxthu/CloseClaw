...
**三阶段（Light / REM / Deep）均为程序化处理**。Dream Diary 是 dreaming 完成后触发的可选 LLM 叙事摘要，不参与升格决策。
...
| 明确性 | owner 明确表述 vs agent 推断 | owner 明确表述显著加分（高于 OpenClaw 默认权值） |
