...
entity type 定义了记忆系统中 entity 的分类体系。Miner 2 在生成 entity 时为每个 entity 指定一种类型，类型决定了该 entity 在 searching 和 dreaming 评分时的权重。
...
**相似度阈值**：用于向量相似度匹配的最低余弦距离门槛，在 Miner 2 分配 entity 和 active-searcher 搜索匹配 entity 时使用。值越高匹配越严格。向量存储方案待 Miner 2 设计落地时确定（如 SQLite 向量扩展或独立向量库）。

### 约束

Miner 2 产出 entity 时，entity 名称不超过 10 个单词。
...
**类型目录生成**：Miner 2 运行前，合并 SQLite `entities` 表和 `entity_types` 表的数据，按 `type` → `normalized_name` 字母序排列，生成固定格式的文本列表，作为 Miner 2 prompt 的一部分。
