...
dreaming 产出 MEMORY.md（可执行的行为规则）和 Dream Diary（可选叙事摘要），不作为独立的记忆存储层——原始 event 和 entity 全部在 SQLite 中。MEMORY.md 是全局共享的单一规则文件，所有 agent 引用同一份（与 per-agent 隔离的 events/entities 不同）；每次注入时读取最新内容，用户手动编辑后下次注入即生效。

### 实体类型

定义 11 种 entity type：time / location / person / organization / subject / product / metric / action / work / group / tags。详见 [entity-types](entity-types.md)。
...
| [entity-types](entity-types.md) | 11 种 entity type 定义 |
...
记忆体系由两条独立的异步链路组成，两者并行运行、互不串行：

### 链路 A：挖掘 → 升格（会话结束 hook 或定时触发）

1. memory-miner 挖掘（两种触发：sub-agent session 结束 hook 或 Daemon DreamingScheduler 定时扫描）
   - Miner 1（LLM session）：输入（transcript + 已有 event 列表 + 已有 MEMORY.md）→ 提取 event（类别 error / anger / decision / insight，error / anger 提炼 lesson）
   - Miner 2（LLM session）：Miner 1 的 event 列表 + 完整 entity/type 目录（SQL → 固定排序文本）→ 分配 entity（名字优先 + 向量兜底匹配）
2. 写入 SQLite
   - events 表：event 持久化（标题、摘要、正文、类别、lesson、来源 session、时间戳、遗忘截止时间 expires_at）
   - entities 表：新 entity 写入（agent_id 隔离 + UNIQUE(agent_id, type, normalized_name) 自动去重）
   - event_entities 关联表
...
1. active-searcher 搜索：当前消息 + 上下文 → 提取查询概念 → SQL entities 索引命中匹配 → 关联 event → 去重过滤（per-session 已注入 ID 集合）→ 浓缩摘要 → 写入 `memory_injection` 槽位（tool role）
   - 特殊角色（memory-miner 自身、dreaming 浓缩 session）不触发 active-searcher
   - 注入命中的 event 小幅延长其 expires_at

### 遗忘（贯穿两链路）

- event 创建时附带 expires_at（遗忘截止时间），耗尽后自动删除
- Miner 1 去重命中（再次识别）→ 大幅延长 expires_at
- active-searcher 注入命中 → 小幅延长 expires_at
...
- **下游**：
  ...
  - System Prompt 静态层：MEMORY.md 通过 [MemoryFragmentProvider](../system_prompt/fragment-provider.md)（实现 [PromptFragmentProvider](../common/core-traits.md#promptfragmentprovider) trait）注入 system prompt 静态层 MemorySection
