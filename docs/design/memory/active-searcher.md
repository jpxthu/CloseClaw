...
3. 概念提取：LLM 从消息中提取查询概念
   ...
   - 场景特征：当前语境的场景描述
4. 搜索
   - 主路径：查询概念 → SQL entities 表匹配 entity（精确/模糊/向量匹配），type weight 加权排序
   - 补量：主路径结果不足时，消息 embedding × entity embedding 向量检索补充
...
4. 去重过滤（排除已注入 event ID）→ 将本轮命中 event ID 加入已注入集合
5. 浓缩为文本摘要（≤ 字符上限）
