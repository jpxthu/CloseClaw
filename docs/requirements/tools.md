...
- 被替换的原文默认在文件中精确匹配到一处。精确匹配失败时尝试模糊匹配，降低因格式差异导致的匹配失败。匹配零处时操作被拒绝。匹配多处时操作被拒绝，除非 Agent 明确要求全文替换
- 同一文件的多次编辑操作串行执行；不同文件的操作互不影响
- Agent 对同一文件的多次编辑应合并到一次调用中完成。分次编辑同一文件时，后一次编辑会因文件已被前一次编辑改动而被拒绝，Agent 需重新读取文件后再编辑
...
- Agent 可以为命令设置超时时间，超时后命令转入后台继续运行，Agent 不会被阻塞等待，完成后收到通知（详见 F5）
...
- session 停止引发的批量终止不发通知——session 已停止，无通知接收方
- 后台任务运行中被交互式提示（如确认对话框）卡住时，Agent 收到警告通知，建议终止该任务并按需重试
- session 停止时，该 session 的所有后台任务进程被终止，已产生的输出保留可读

> **交叉引用**：通知的优先级（next/later）与 session 消息队列的对接，详见 [session §F9](session.md)（消息注入）。
...
- 疑似攻击或结构不确定的命令单独记录日志到 Agent workspace 之外的专用目录，日志包含触发消息与完整对话记录的引用，供安全调查
