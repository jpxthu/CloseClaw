...
图片进入上下文的机制：现代 LLM 除文本 token 外支持图片输入——图片经模型自身的视觉编码器处理，不需要也不应该转成文字。读取图片后，工具返回图片内容块（多模态消息内容的一种，与文本块并列），携带图片的二进制编码与媒体类型，由 LLM 层随对话消息一并发给模型；多模态能力的模型直接"看图"。当前模型不支持图片输入时，读取被拒绝并告知原因。图片不需要 Agent 转逃描述——看图、比较图、识别图中文字都由模型直接完成。
...
> 读取文本文件，返回带行号的内容。
> 读取图片文件（PNG/JPEG/WebP/GIF），返回图片本身——系统在下次模型请求前自动校验并降采样过大的图片，直接用本工具看图，不要为此安装图像库或制作缩略图；需要当前模型支持图片输入，多个独立图片可小批量并发读取。
...
> 文件不存在：返回错误。
> 文件存在但内容为空：返回空文件提示，不是错误。
> 当前模型不支持图片输入：图片读取被拒绝，说明原因。
...
> 未读取就修改：「文件尚未读取——先读取，再重试」。
> 读取后文件被外部改动：「文件已变更——重新读取后再重试」。
> 替换原文匹配零处：告知未找到指定文本，文件内容可能与预期不一致。
> 替换原文匹配多处：告知匹配数量，提示缩小原文范围或明确全文替换。
