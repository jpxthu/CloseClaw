...
本节各工具面向 Agent 的说明提示词原文（产品定义，开发不改写）：

文件读取：

> 读取文本文件，结果每行带行号。默认读整个文件；已经知道要看哪段时只读那段——用行号范围分段读取大文件，需要完整内容时从上次结束处续读。读文件用本工具，不要用 shell 命令（cat/head/tail）。读取不存在的文件会返回错误——路径来自用户时假定有效，报错后向用户确认即可，不是异常。文件存在但内容为空时，返回明确的空文件提示而不是错误。

图片读取：

> 图片以图片内容块进入模型上下文——与文本结果并列的独立多模态内容块，不是文本编码。读取前自动压缩（长边不超过约 2000 像素、编码后不超过模型单请求上限）；当前模型不支持图片输入时读取被拒绝并告知原因。

目录浏览：

> 列目录用本工具，不要用 shell 的 ls/find。返回条目标注类型（文件/目录/符号链接）和基本信息，超出数量上限时截断并告知。

文本搜索：

> 按文件名查找用路径模式匹配，按内容查找用正则搜索——不要用 shell 的 find/grep/rg，专用工具对权限和输出做了优化。命中结果带行号、有数量上限；需要上下文时读取命中文件。开放式多轮检索优先委派子 Session。
...
> 写文件用于创建新文件或完全重写。已存在的文件必须先读取过才能覆盖——未读先写会被拒绝。局部修改优先用编辑工具——它只传输差异，不需要重发整个文件。除非用户明确要求，不要创建文档类文件（README 等）、不要在文件里加 emoji。

文件编辑：

> 编辑前必须先读取过目标文件。要替换的原文必须在文件中唯一匹配——不唯一时扩大原文范围带上上下文，或明确要求全文替换。从带行号的读取结果复制原文时，行号前缀不是文件内容——永远不要把行号前缀写进替换文本。优先修改已有文件，除非确有必要不要新建文件。除非用户明确要求，不要在文件里加 emoji。

多个替换在一次编辑内完成：

> 同一文件的多处独立修改，合并为一次编辑调用中的多个替换条目，不要发多次编辑调用。所有替换基于文件原始内容匹配，互不重叠；相邻的修改合并为一条，不要为了"连接远处修改"携带大段未变内容。原文片段保持最小——能唯一定位即可，不要为了保险带上大段上下文。

版本控制操作：

> 查看变更状态、提交历史、提交修改、推送拉取用本工具，不要手工拼 git 命令。提交信息描述真实变更内容，不编造未做的事。
