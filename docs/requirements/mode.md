...
**斜杠指令**：通过 `/execute` 命令触发执行。`/execute` 不接受参数，无 Immediate 标记——需等待当前 LLM 调用结束后执行。若当前处于 Plan Mode 则先退出再进入 Auto Mode，否则直接进入 Auto Mode。

> **交叉引用**：`/execute` 的指令注册与分派由 [slash §F2](slash.md)（模式切换指令）管理。
...
执行过程中的中断可以无缝恢复。暂停和恢复通过自然语言触发，无专用斜杠指令——User 说"停一下""暂停"即暂停执行，说"继续""继续执行""继续 xx 步"即恢复执行。暂停/恢复的语义均为 prompt 注入，无额外系统机制。
