...
- Plan Mode：Agent 只做规划不做执行，工具集受限为只读（仅 plan 文件可写）。User 可以反复要求 Agent 修改 plan，Plan Mode 没有审批栅栏——不自行退出，由 User 决定何时退出
- Auto Mode（执行模式）：Agent 连续自主执行 plan 步骤，不等 User 逐步确认，但危险操作仍需 Owner 审批。可直接进入，不需要先经过 Plan Mode

模式切换规则：
- Plan Mode 与 Auto Mode 是独立模式，可分别进入和退出
- Plan Mode 下 User 通过 `/execute` 命令或自然语言表达执行意图（如"执行吧""开始实行"）时，退出 Plan Mode 并进入 Auto Mode；自然语言路径需指定 plan 名称并经确认（详见 F4）
- Auto Mode 下所有任务完成后自动退出并恢复默认模式

> **交叉引用**：`/plan`、`/mode` 模式切换指令的完整语法和参数由 F14（模式切换指令）定义；`/execute` 的定义见 F4（执行触发）。指令拦截和分派由 [slash §F1](slash.md)（斜杠指令入口）定义。
...
- Agent 自行判断模糊点是否消除，消除后进入标准路径的 Review 和 Final Plan 阶段
...
> **交叉引用**：plan 命名格式属于 Mode 配置项，变更后自下一次 plan 创建起生效；配置重载机制详见 [config §F4](config.md)（配置重载）。
...
- 常规决策自主做出，不上报给 User
- 不在执行中途主动重新进入 Plan Mode（User 显式选择回退修改 plan 的情形除外，详见 F12）
- 接受 User 随时发来的修正建议
...
- 每条审批记录：工具名、操作描述、最终处置（批准/拒绝）、时间戳
- 按时间倒序排列
- 持久化，跨 Session 可见
- User 可以配置日志存储上限，变更后自下一条日志写入起生效

> **交叉引用**：日志上限属于 Mode 配置项，配置重载机制详见 [config §F4](config.md)（配置重载）。
...
plan 写完后，执行方式完全由 User 决定，没有固定的模式约束。
...
- 对话被压缩或系统重启后 Agent 仍然知道当前执行进度（当前第几步、哪些已完成、哪些未完成），不需要从第 1 步重新执行
...
当 plan 的全部步骤处于终态（已完成 `[x]`、失败 `[!]` 或已跳过 `[~]`）时，若该 plan 文件最后访问时间超过配置天数，则自动归档，避免 plans 目录无限增长。User 可以配置归档天数，变更后自下一次归档扫描起生效。

### F14. 模式切换指令

User 通过以下斜杠指令查询或切换 Session 运行模式：
...
模式切换不立即生效——切换仅标记 Session 状态，下一条用户消息前才应用新模式的约束。
...
- **可配置性**：审计日志上限、归档天数、plan 命名格式均支持 User 按偏好配置
