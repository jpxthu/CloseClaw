...
Owner/User 可以切换 Agent 的运行模式来控制 Agent 的行为边界——规划时只读不执行，执行时自主推进但不越安全红线。Plan 和执行是两个独立的事情：Plan 写完后可以在同一 session 内执行（继承规划上下文），也可以开新 session 执行（从 plan 文件读取背景）。

## 功能需求

### F1. 运行模式

Agent 在 session 内可运行于以下模式，模式决定了 Agent 可用的工具集、行为约束和权限边界。
...
- Auto Mode（执行模式）：Agent 连续自主执行 plan tasks，不等 User 逐步确认，但危险操作仍需 User 审批。可直接进入，不依赖 Plan Mode 审批

模式切换规则：
- User 通过斜杠命令显式进入 Plan Mode 或 Auto Mode
...
阶段切换由 Agent 自行判断，阶段之间无系统卡点。Review 不是一次性审批——User 可以反复审阅并给出修改意见，Agent 不断调整 plan，直到 User 满意并决定执行。Research 和 Design 阶段 Agent 可以 spawn 子 Agent 并行工作；spawn 出的子 Agent 继承 Plan Mode 的只读约束，不可执行写操作。
...
**斜杠指令**：
- `/execute <plan名称>`：切换到 Auto Mode 并开始执行指定 plan
- plan 名称后可附加进入执行模式后的第一条消息，如 `/execute my-plan 只做第一步`
...
**执行路径**：
- 同 session 执行：当前 session 退出 Plan Mode（如处于 Plan Mode），进入 Auto Mode，继承规划上下文
...
步骤的状态流转：未开始 → 进行中 → 已完成 / 失败 / 已跳过。失败后允许重试（失败 → 进行中）。已完成不允许回退。已完成两步后 User 发现设计有问题，可以回 Plan Mode 修改后两步，不影响已完成步骤。
...
Agent 在 Auto Mode 下以连续自主方式执行 plan tasks。
