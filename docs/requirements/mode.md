...
**斜杠指令**：通过 `/execute <plan名称> [附加指令]` 命令触发执行。`plan名称` 为必选参数。`附加指令` 可选——空格后的所有内容作为一条用户消息注入 Auto Mode 的初始对话。`/execute` 为非 Immediate 指令。若当前处于 Plan Mode 则先退出再进入 Auto Mode，否则直接进入 Auto Mode。

> **交叉引用**：`/execute` 的指令注册与分派由 [slash §F2](slash.md)（模式切换）管理。

**自然语言**：Agent 拥有执行触发工具，参数与 `/execute` 相同（`plan名称` + 可选附加指令）。该工具调用时自动向 User 发起确认（y/n），确认交互不经过 LLM——系统直接拦截并展示确认卡片。User 确认后进入 Auto Mode 开始执行。
