...
- User 可以配置 Interview 最大轮数，达到上限后 Agent 退出 Interview 循环并提示 User 决定：继续澄清、放弃规划、或直接写入当前 plan；未设置上限时无硬性限制
...
- 每步完成后 Agent 在 plan 文件中标记对应步骤为已完成（如 `[x]`）
- 步骤标记格式由 Agent 一致遵循，确保可读性
...
- **可配置性**：Interview 最大轮数、拒绝日志上限、归档天数均支持 User 按偏好配置
