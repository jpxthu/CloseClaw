...
**斜杠指令**：通过 `/execute <plan名称> [附加指令]` 命令触发执行。`plan名称` 为必选参数。`附加指令` 可选——空格后的所有内容作为一条用户消息注入 Auto Mode 的初始对话。`/execute` 为非 Immediate 指令。若当前处于 Plan Mode 则先退出再进入 Auto Mode，否则直接进入 Auto Mode。`/execute` 是 Gateway 层拦截的显式系统指令，无需 User 二次确认。

> **交叉引用**：`/execute` 的指令注册与分派由 [slash §F2](slash.md)（模式切换）管理。

**自然语言**：Agent 拥有执行触发工具，参数与 `/execute` 相同（`plan名称` + 可选附加指令）。该工具调用时自动向 User 发起确认（y/n），确认交互不经过 LLM——系统直接拦截并展示确认卡片。User 确认后进入 Auto Mode 开始执行。自然语言路径需要确认，因为 LLM 可能在非执行意图的上下文中调用该工具。
...
plan 文件命名需包含任务识别信息。命名格式由 User 在配置中一次性指定——时间戳格式（如 `20260718-2006-任务名`）或随机词组格式，创建新 plan 时自动套用所选格式。
...
`/plans` 无 Immediate 标记。不接受参数时列出所有 plan 及其步骤完成情况。`/plans <plan名称>` 查看特定 plan 的完整内容。
...
> **交叉引用**：`/plans` 的指令注册与分派由 [slash §F12](slash.md)（plan 浏览）管理。
...
执行过程中的中断可以无缝恢复。暂停和恢复通过自然语言触发，无专用斜杠指令——User 说"停一下""暂停"即暂停执行，说"继续""继续执行""继续 xx 步"即恢复执行。暂停和恢复均由 User 在对话中发出的自然语言指令触发，系统不做特殊处理。
...
全部步骤处于终态（所有步骤标记为已完成 `[x]`、失败 `[!]` 或已跳过 `[~]`，即 Tasks 节中不存在未开始 `[ ]` 或进行中 `[-]`）的 plan 文件，在最后访问超过一定天数后自动归档，避免 plans 目录无限增长。User 可以配置归档天数。
