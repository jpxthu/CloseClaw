...
- 归档过程中系统崩溃时，正在归档的会话的未完成操作同样被识别和恢复
...
- 超过最长时间后，系统解除等待并通知父 Session 部分子 Session 尚未完成；父 Session 可自行决定继续等待或调查问题
...
- 非用户消息（子 Session 完成通知、后台工具结果、记忆注入等）与用户消息沿用同一阻塞框架：Agent 推理或等待同步工具时按序排队（非用户消息优先注入），有后台任务或子 Session 时立即注入
