...
用户与 Agent 的对话自动持久化，系统重启后 Agent 能接续之前的对话。每轮对话的历史消息完整保留。
...
- 手动压缩：`/compact <保留指令...>`，例如 `/compact 保留用户名和邮箱`）
...
- 子 Agent 在可配置时间内未完成时，向父 Agent 推送超时通知，父 Agent 可决定继续等待、终止或重试
...
- 流式输出：Agent 回复经出站管道实时渲染后逐步呈现给用户，Thinking 内容默认隐藏。流式响应出错时，已渲染给用户的内容保留可见，不完整回复不写入消息历史
...
- 系统崩溃时，自动识别未完成的工具调用、子 session 生成和出站消息。出站消息自动重投递，其余操作注入恢复通知由 Agent 决策（详见 F1）
