...
- 子 Session 完成后，其结果以完成通知的形式通过统一消息队列注入父 Session（带去重保护）；该通知与其他消息类型一视同仁，统一按 F10 的排队规则处理，Agent 不需要轮询
- 子 Session 超时预警通知：
...
- 系统对活跃 Session 和对话历史做双向一致性校验——有活跃 Session 标记但无对应对话历史，视为损坏并清理；有对话历史但无对应 Session 标记，视为孤立并清理
