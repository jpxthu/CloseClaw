...
Session 模块满足用户对 Agent 对话上下文的持久化、可恢复、可管理需求——每次对话自动保存，重启不丢失，闲置自动清理。
...
- 会话路由键由「平台 + 发送者 + 对端 + 账号」四元组组成。同一路由键下可以有多个历史 session（由 `/new` 创建），当前活跃的是最新（last_message_at 最大）的一个
...
Owner 通过 workspace 中的 bootstrap 文件（AGENTS.md、SOUL.md 等）定义 Agent 的行为规则和身份。这些配置在每次 session 创建、归档恢复或对话压缩完成时自动注入 Agent 的 system prompt。
...
- 自动压缩：每次对话后检测 token 用量，在剩余空间低于阈值时自动触发
- 压缩只处理对话消息（user/assistant），不触碰 system prompt（角色定义始终完整）
- 压缩结果为一条 boundary 消息，包含对话的结构化摘要（TODO：压缩维度待后续细化）
...
- 任何用户都可以通过 `/stop --cascade` 强制终止当前 session 及所有子 session
...
- 推理深度支持四档（Low / Medium / High / Max）和 off 状态，通过 `/reasoning [level|off]` 指令控制，`/reasoning` 查询当前档位
- Max 档在模型不支持时自动降为 High，`/reasoning` 查询显示实际生效值
- Owner 可以通过配置设置默认推理深度，运行时指令覆盖配置默认值
- 流式输出：Agent 的回复实时逐字推送给用户。流式响应中途出错时，已推送的部分保留不动，但不将不完整回复写入对话历史
- 用户可以查看跨轮次的 token 消耗统计和缓存命中率
- Agent 的内部推理过程（Thinking 内容）默认对用户隐藏，但保留在消息历史中供后续对话参考
...
- 每个 Agent 可独立配置空闲时间和清理时间，主 Agent 和子 Agent 分别设置
