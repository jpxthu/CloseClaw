...
- 会话路由键由平台、会话发起方、对端、账号四个维度共同决定。同一路由键下可以有多个历史 session，当前活跃的是最后活跃时间最近的一个

> **交叉引用**：新 session 由 `/new` 指令触发创建，详见 [slash §F3](slash.md)。
- 归档的 session 被访问时自动恢复，恢复时用户收到「正在恢复会话…」提示，恢复后 Agent 的 system prompt 按最新配置文件重新注入
- 系统重启时，自动扫描所有活跃 session，对有未完成操作（工具调用、子 Session 委托、未发出的出站消息）的 session 注入恢复通知。崩溃前未发送的出站消息自动重投递
...
- 子 Session 超过硬超时时，系统终止该子 Session 并级联终止其所有后代，并通知父 Session
- 暂停等待有超时保护：系统取所有等待中子 Session 的硬超时最大值，加上缓冲（默认 1 分钟）作为 yield 的最长等待时间
- 以下情况提前解除等待：用户发送新消息、所有子 Session 正常完成、子 Session 超时预警、子 Session 硬超时终止
- 超过最长时间后 yield 自动解除，通知父 Agent 部分子 Session 可能超时，父 Agent 可自行决定继续等待或调查问题
...
- 可配置的自动质量检查（轻量 LLM 门禁），检测 Agent 是否只计划不执行、是否陷入工具调用死循环
- 执行对话历史的破坏性操作（压缩、system prompt 修改）前自动创建对话备份，异常时可回滚到上一个安全状态
- 系统异常中断时，自动识别未完成的工具调用、子 Session 创建和出站消息。出站消息自动重投递；其余操作注入恢复通知，由 Agent 决策如何处理（详见 F1）
