...
- User 发送消息时，系统自动查找该对话对应的 Session——若存在活跃 Session 则复用；否则若存在归档中或已归档 Session，则恢复其中最后活跃时间最新的一个；否则创建新 Session
- 用户对话 Session 由平台、发送者、会话对端、账号四项标识共同确定；同一组标识下可有多个历史 Session，当前活跃的是最后活跃时间最新的一个。字段定义见 [im_adapter §F2](im_adapter.md)（入站消息归一化）
...
> **交叉引用**：强制开启新 Session 由 `/new` 指令触发（即使当前标识下已有活跃 Session）。详见 [slash §F3](slash.md)（Session 管理）。
...
- 手动压缩：User 可手动触发压缩，并可指定需要重点保留的内容
...
- inactive 的 Session 自动归档：进入「归档中」，随即不再是活跃 Session；归档完成后标记为「已归档」
...
- 有未完成的子 Session 时，父 Session 不被判定为 inactive，不触发归档。若因系统错误被归档，系统记录告警日志，其未完成子 Session 的完成通知丢弃
...
- **子 Session 未完成**：Session 有未完成的子 Session（已创建、尚未完成）
