...
- 自动压缩（阈值触发）保留最近一段对话原文，仅将其余更早的对话压缩为摘要。保留区比例以「保留原文的 token 数占当前 Session 上下文窗口的百分比」衡量（默认 16%、每个 Agent 可独立配置），保留方式为自最近消息向前累计至该预算；被保留的原文原样保留、不进入摘要。压缩不拆散工具调用与其结果
...
- **可配置性**：inactive 时长、清理时长按 Agent 独立配置，且主 Agent Session 与子 Session 可分别设置；压缩的告警阈值 / 压缩阈值 / 保留区比例 / 熔断失败次数 / 摘要模型、子 Session 超时预警时间 / 间隔比例按 Agent 独立配置；各配置项独立回退到系统默认值。配置变更的生效时机见 F3、F6 与 [config §F4](config.md)（配置重载）
