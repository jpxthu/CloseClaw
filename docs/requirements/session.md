...
- User 发送消息时，系统自动查找该对话对应的 Session——若存在活跃 Session 则复用，若已归档则恢复该标识下最后活跃时间最新的一个，否则创建新 Session
- Session 由平台、发送者、会话对端、账号四项标识维度共同标识，字段定义见 [im_adapter §F2](im_adapter.md)（入站消息归一化）。相同标识下可以有多个历史 Session，当前活跃的是最后活跃时间最新的一个
- Session 在 Agent 范围内隔离：消息先路由到接收该消息的机器人所绑定的 Agent（见 [gateway §F4](gateway.md)（普通消息路由到对话）），再按上述四项标识维度查找 Session
...
- 连续压缩失败达到配置的失败次数后自动进入保护暂停（仅阻止自动压缩再次触发，不影响活跃判定和归档），手动 `/compact` 成功后自动解除保护暂停
...
- 子 Session 完成后，其结果以完成通知的形式通过统一消息队列注入父 Session（带去重保护）；该通知与其他非 User 消息类型一视同仁，统一按 F10 的排队规则处理，Agent 不需要轮询
- 子 Session 超时预警通知：
  - 子 Session 的运行时长达到设定的超时预警时间时，系统向父 Session 注入超时预警通知。通知内容包含：设定的超时预警时间、已运行时长、上下文窗口使用情况及 token 用量
...
> **交叉引用**：子 Session 完成通知的排队规则详见 [F10](#f10-消息排队)（消息排队）。
...
- inactive 判定条件：四维活跃维度均为否，且距上次 User 活动超过配置的 inactive 时长，详见 [F11](#f11-session-活跃维度)（Session 活跃维度）
...
- 系统定时扫描子 Session（F4 的超时预警通知不替代本项僵死兜底——父 Agent 选择继续等待的，仍受僵死判定保护）：
...
- 工作目录的取值由解析顺序确定：创建时按解析顺序确定初值，恢复时按同一解析顺序重新确定
...
后台任务结果、记忆注入和子 Session 完成通知注入消息队列，Agent 在后续轮次中按常规对话流程处理。
...
User 消息按以下排队规则注入。排队条件使用 F11 的四维活跃维度。
...
- 非 User 消息（子 Session 完成通知、后台任务结果、记忆注入等）与 User 消息沿用同一阻塞框架：
  - 满足排队条件时：同批积压的非 User 消息按优先级（now > next > later）优先注入，同一优先级内按到达时间先后；未携带优先级的非 User 消息视为同一默认优先级。随后注入排队中的 User 消息
...
- **可靠性**：对话历史在系统重启或进程异常退出后完整保留，已写入历史的消息不丢失。正在执行的操作（工具调用、子 Session 委托、出站消息）在崩溃后能被识别——未发送的出站消息自动重投递，为其余操作注入恢复通知，由 Agent 决定后续处理
