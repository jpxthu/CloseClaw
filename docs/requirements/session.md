...
- inactive 判定条件：四维活跃维度均为否（详见 F11），且距上次用户活动超过配置的 inactive 时长
...
- **可靠性**：对话记录在系统重启或进程异常退出后完整保留，已写入历史的消息不丢失。正在执行的操作（工具调用、子 Session 委托、出站消息）在崩溃后能被识别并通知，由 Agent 决定后续处理
- **可恢复性**：系统重启后自动恢复所有活跃 Session。恢复耗时与当前活跃 Session 数量成正比，复杂度 O(A)，与历史归档会话总量无关
- **性能**：Agent 回复实时逐字展示，首 token 延迟与历史会话总量无关。后台维护任务（归档扫描）不阻塞用户对话的响应
- **可配置性**：每个 Agent 的 inactive 时长、清理时间可独立配置，主 Session 与子 Session 可以分别设置；各配置项独立回退到系统默认值
- **会话独立性**：会话路由、委派、归档恢复等日常操作对历史会话总量的复杂度为 O(1) 或 O(log N)，响应速度不随历史会话总量增加而退化，不对全量历史会话做 O(N) 遍历
- **长期运行稳定性**：系统累计委派大量子任务后，委派新子任务和用户对话的响应速度不随已完成子任务数量增加而退化。已完成子任务的结果归档后仍可查，但不持续占用运行资源；内存占用与活跃会话数量成正比，对历史会话总量为 O(1)
