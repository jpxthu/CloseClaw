...
- Session 在 Agent 范围内隔离：消息先路由到接收该消息的机器人所绑定的 Agent（见 [gateway §F4](gateway.md)（普通消息路由到对话）），再按上述四维标识查找 Session
...
- **Session 独立性**：Session 路由、委派、归档恢复等日常操作对 N 的复杂度为 O(1) 或 O(log N)，不对全量历史 Session 做 O(N) 遍历
