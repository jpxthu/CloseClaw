...
- 压缩只处理用户与 Agent 的对话消息，system prompt 内容完整保留
...
- Agent 可以创建子 Session 来委托子任务。子 Session 的任务描述注入为 system prompt 的一部分，不属于对话消息，压缩时不受影响
...
用户控制 LLM 调用的推理强度。Agent 的回复实时流式推送给用户。
...
- inactive 判定条件：四维活跃维度均为否（详见 F11），且距上次用户活动超过配置的 inactive 时长。四个活跃维度由 F11 定义
...
- 有活跃子 Session（child_active 为 true）时，父 Session 不被判定为 inactive，不触发归档。若因系统错误被归档，记录告警日志，该子 Session 的完成通知丢弃
- 系统对活跃 Session 和对话记录做双向一致性校验——有活跃 Session 标记无对应对话记录视为损坏并清理，有对话记录无对应 Session 标记视为孤立并清理
...
- 系统定时扫描未完成的子 Session：
  - 已完成（四个活跃维度均为否且已产出最终 assistant 消息）但完成通知未成功送达父 Session 的，补推完成通知（若父 Session 已归档则跳过）
  - 非已完成状态且超过五分钟无新产出（无新 assistant 消息、无工具执行结果变化）的，判定为僵死，自动终止（级联终止其所有后代）并向父 Session 注入僵死通知
...
- 记忆注入借助 Session 通道完成，具体内容与注入位置由 memory 模块定义
...
Session 在任意时刻可以在多个维度上同时处于活跃状态，每个维度独立开启或关闭。活跃维度由 Session 维护。
...
Session 模块在以下环节记录调试日志（复用现有日志机制，不额外产生重复记录）：
- 会话创建、查找、归档恢复
- 对话轮次追加与历史截断
- 上下文压缩事件（触发原因、压缩前后 token 数）
- 活跃维度变化（任一维度开启或关闭）
- 子 Session 创建与完成
- 消息注入事件（后台任务结果、记忆注入）
- 健康检查与异常检测结果
...
session 空闲作为四维复合状态的典型代表，不在 session 模块集中定义——各功能模块按需从四维组合自己的判定条件，具体组合语义（合取/析取、是否附加时间窗等）由消费方自行决定：
