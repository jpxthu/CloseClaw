...
跳转规则按顺序匹配，第一个条件满足的规则即生效。必须有兜底分支（default）确保总是有出路。条件评估方式可预测：给定 agent 对跳转问题的回答，Engine 按预定义规则产出确定的结果（布尔比对、枚举匹配、字符串比对），不依赖 LLM 做语义判断。
...
退出后的 session 不再受 workflow 约束，agent 恢复为普通 session 的自由对话模式。
