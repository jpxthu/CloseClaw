...
- 文本消息进入正常对话链路
- 图片消息以图片内容进入 LLM 上下文（大小超过阈值时的引用形式见 [im_adapter §F9](im_adapter.md)（媒体文件收发与存储））
- 文件、音频等其它非文本消息：以媒体引用进入 LLM 上下文，Agent 通过引用使用媒体内容（媒体存储与引用机制见 [im_adapter §F9](im_adapter.md)）
...
- 网关重启执行期间，入站新消息全部暂存，重启完成后按原到达顺序补投（暂存与补投的触发时机详见 [daemon §F6](daemon.md)（配置触发的网关重启））
