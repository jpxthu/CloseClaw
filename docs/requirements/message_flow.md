...
5. 空消息丢弃、消息类型识别；非斜杠文本消息进入 LLM 对话流程，以 `/` 开头的消息由斜杠指令流程处理。
...
1. 流式输出按格式自动选择渲染：纯文本以轻量文本消息发送，含格式特征的内容以富格式发送（飞书渲染为卡片）。
...
同一条群消息同时 @ 多个机器人时，每个接收方机器人各自收到一份平台事件；事件中的发送者标识与 @ 对象标识按接收方应用语境映射，互不相同。这些标识必须由身份映射归并为同一个 CloseClaw User，否则同一用户会在不同机器人下被识别为不同 User 并各自开启 Session。
