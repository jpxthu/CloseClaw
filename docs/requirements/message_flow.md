...
4. 发送者经身份映射解析为 CloseClaw User（该结果即账号标识），走「IM 用户 → User」入站方向。
5. 空消息丢弃、消息类型识别；不以 `/` 开头的文本消息进入 LLM 对话流程（以 `/` 开头的消息由斜杠指令流程处理）。
6. 消息路由到 Agent——走「机器人 → Agent」入站方向，由接收方机器人解析出路由目标 Agent。
7. 在该 Agent 范围内按四项标识（平台标识、发送者、会话对端、账号标识）共同确定 Session，按下列情形之一处理：
...
上述跨应用 ID 隔离与归并属大量实测才确定的平台行为，以真实采集的平台数据为准；飞书 CLI 实测样本（含同一消息被两个机器人分别接收的跨应用 ID 对照事件）见 `tests/fixtures/feishu/cli-poc/`。
