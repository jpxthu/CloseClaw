...
- **user-invocable**：控制该技能是否出现在技能清单中。默认不出现；声明后该技能出现在清单中（但声明了 paths 的技能遵循 F6 条件激活规则，不在初始清单中）
...
> **交叉引用**：SP 组装结果在事件之间不变，确保模型服务端缓存持续命中，详见 [system_prompt §F7](system_prompt.md)（API 前缀缓存利用）。
...
Agent 可通过内置技能获得创建技能文件的指导。User 在对话中描述需求后，Agent 按该技能的指令创建符合规范的 SKILL.md 文件。
