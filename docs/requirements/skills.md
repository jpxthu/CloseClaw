...
> **交叉引用**：注入机制详见 [system_prompt §F2](system_prompt.md)（工具与技能清单注入）、[system_prompt §F7](system_prompt.md)（API 前缀缓存利用）。
...
> **交叉引用**：生效时机由 session 生命周期驱动。详见 [session §F2](session.md)（Agent 角色与能力配置）、[system_prompt §F6](system_prompt.md)（内容缓存与自动刷新）。
...
> **交叉引用**：详见 [system_prompt §F7](system_prompt.md)（API 前缀缓存利用）。
...
- **响应稳定性**：技能清单的注入和更新不得导致 Agent 对话质量下降或历史对话丢失。详见 [system_prompt §F7](system_prompt.md)
