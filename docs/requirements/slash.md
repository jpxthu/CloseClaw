...
模式切换指令（`/plan`、`/mode`）的完整语法、参数和业务行为由 mode 模块定义。slash 模块仅提供 Gateway 层的指令拦截和分派机制（见 F1）。

> **交叉引用**：指令参数和模式切换语义详见 [mode §F14](mode.md)（模式切换指令）。模式类型定义和 Agent 行为约束详见 [mode §F1](mode.md)（运行模式）。
