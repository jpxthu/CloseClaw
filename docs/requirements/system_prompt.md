...
Owner 在 Agent 的 Bootstrap 文件目录下通过一系列配置文件（统称 Bootstrap 文件）定义 Agent 的身份、操作规程、工具使用指南和 Owner 偏好。其中身份由角色定义和身份声明构成，Owner 偏好对应 Owner 信息文件。Bootstrap 文件属于 Agent 自身配置，同一 Agent 对全部 User 表现一致的人格。这些 Bootstrap 文件在 Agent 每次 Session 启动时自动加载，作为 System Prompt 的核心组成部分。
...
- 边界说明：Agent 的唯一 ID 与显示名称（[agent §F1](agent.md)（Agent 配置档案）中的「身份标识」字段）是配置档案中的标识字段，供系统内部规则化处理使用（如配置查询、spawn 目标定位），不属于 Bootstrap 文件，不注入 System Prompt。Bootstrap 文件均为注入 System Prompt 的 prompt 内容——内部标识与 prompt 内容是两类不同的对象
...
- **无 Bootstrap 文件的 Session**：仅加载工具清单，不加载任何 Bootstrap 文件与长期记忆。触发条件二选一：Agent 配置档案显式声明无 Bootstrap 文件（见 [agent §F1](agent.md)（Agent 配置档案）），属于正常的功能场景；或 Agent 未声明但 Bootstrap 文件目录缺失，此时同样按本类型兜底运行以保证对话不中断，但该配置异常需记录降级告警日志（见 [debug_log §F2](debug_log.md)（分层日志级别））

> **注**：F4 运行时上下文（频道、工作目录）对所有 Session 类型均加载。各条目中的「工具清单」均指 F2 定义的注入内容，含工具清单与技能清单。
