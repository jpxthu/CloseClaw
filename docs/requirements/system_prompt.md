...
> **交叉引用**：bootstrap 文件的加载模式（Full/Minimal）和所在目录路径，由 [agent §F1](../requirements/agent.md)（配置档案）、[agent §F2](../requirements/agent.md)（身份与人格分离）定义。会话创建/恢复/上下文压缩完成时的重建触发，见 [session §F1](../requirements/session.md)（对话持久化与恢复）、[session §F3](../requirements/session.md)（长对话压缩）。
...
> **交叉引用**：重建触发的外部事件来源——会话恢复见 [session §F1](../requirements/session.md)，上下文压缩完成见 [session §F3](../requirements/session.md)。Agent 配置文件变更的检测机制由 [agent §F6](../requirements/agent.md)（运行时配置查询）管理，本模块作为消费方响应。
