...
- **工具清单**：工具清单的字段内容、常用/延迟分组、危险度标记、长度截断与探索提示由 [tools §F1](tools.md) 定义。本模块通过 ToolsFragmentProvider 消费渲染结果
- 当工具定义发生变更时，清单自动更新
...
> **交叉引用**：动态指令的追加、查看、清除基础行为，见 [session §F2](session.md)（Agent 角色与能力配置）。本节仅定义 System Prompt 内容层的专属行为。
...
> **交叉引用**：重建触发的外部事件来源——会话恢复见 [session §F1](session.md)，重建触发见 [session §F2](session.md)（Agent 角色与能力配置），上下文压缩行为见 [session §F3](session.md)。Agent 配置文件变更的检测机制由 [agent §F6](agent.md)（运行时配置查询）管理，本模块作为消费方响应。
