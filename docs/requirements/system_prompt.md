...
> **交叉引用**：会话类型的判定（主/子/无 workspace）和子 Agent 的 spawn 参数，由 [agent §F7](agent.md)（子 Agent 创建）定义。各类型对应的 System Prompt 加载范围由本模块定义。
...
> **交叉引用**：检查点的存储格式和写入机制，见 [session §F1](session.md)（对话持久化与恢复）。本模块仅声明动态指令的持久化约束。
