...
System Prompt 是每次与 AI 模型通信时发送的引导前缀，承载 Agent 的身份与行为准则、工具与技能清单、长期记忆、运行时上下文以及 Owner 动态追加的指令。Owner 需要：通过 workspace 配置文件定义 Agent 的行为准则，让 Agent 了解自己有哪些工具和技能，在不同类型的会话中加载恰当的上下文，并能随时追加临时指令，同时确保静态内容高效缓存、文件变更时自动反映最新内容。
...
Agent 需要在 System Prompt 中看到当前可用的工具清单和技能清单，以便在对话中正确选择和使用。本节工具清单与 F1 中列出的工具使用指南（bootstrap 文件之一）相互独立——前者是各工具的具体功能说明（由系统生成），后者是 Owner 对工具使用规范的高层约束（由 Owner 编写）。

- **工具清单**：工具清单的字段内容、常用/延迟分组、危险度标记、长度截断与探索提示由 [tools §F1](tools.md) 定义。本模块通过 ToolsFragmentProvider 消费渲染结果
- **技能清单**：列出所有可用技能的摘要。清单的生成规则、排序、过滤和条件激活标记由 [skills §F4](skills.md)（技能清单）定义。本模块通过 SkillsFragmentProvider 消费渲染结果
- 当工具定义或技能文件发生变更时，清单自动更新。技能变更的触发和生效机制详见 [skills §F5](skills.md)（热重载）
- 技能正文内容在模型决定使用时按需注入，不预先进入 System Prompt

> **交叉引用**：当前 Agent 可用的工具/技能范围（白名单/黑名单），由 [agent §F3](agent.md)（Agent 能力组合）定义。本模块负责将可用清单渲染为 System Prompt 中的分组描述文本。
