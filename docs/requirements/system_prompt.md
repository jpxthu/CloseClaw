...
System Prompt 是每次与 AI 模型通信时发送的引导前缀，承载 Agent 的身份与行为准则、工具清单、技能清单、长期记忆、运行时上下文以及 Owner 动态追加的指令。System Prompt 仅在以下事件触发时组装：新会话创建、归档恢复、上下文压缩完成后。组装结果在事件之间不变，确保 KV Cache 稳定性。Owner 通过 workspace 配置文件定义 Agent 的行为准则，让 Agent 了解自己有哪些工具和技能，在不同类型的会话中加载恰当的上下文，并能随时追加临时指令。
...
- System Prompt 所有组成部分按以下固定顺序组装，每个部分在配置不变时内容确定，多次组装结果逐字节相同：
  1. AGENTS.md（操作规程）
  2. SOUL.md（角色定义）
  3. IDENTITY.md（身份标识）
  4. USER.md（Owner 信息）
  5. TOOLS.md（工具使用指南）
  6. BOOTSTRAP.md（自定义引导指令，可选，取决于会话类型和加载模式）
  7. 工具清单（系统生成，内容由 [tools §F1](tools.md) 定义）
  8. 技能清单（系统生成，内容由 [skills §F4](skills.md) 定义）
  9. 长期记忆（系统生成，来源为 MEMORY.md，可选，取决于会话类型和加载模式）
  10. 频道上下文（动态层，每次请求即时获取）
  11. 工作目录（动态层，每次请求即时获取）
  12. 模式指令（动态层，取决于当前 mode）
  13. Git 状态（动态层，可配置关闭，默认关闭）
  14. Owner 追加的指令（动态指令区）
- 不存在或不适用的部分跳过，不影响其余部分的位置和顺序
...
- 工具清单在每次 System Prompt 组装时从工具注册中心重新生成
- 技能清单的注入位置见 F1 组装顺序第 8 项，组装时机和清单内容由 [skills §F4](skills.md)（技能清单）定义。技能正文内容在模型决定使用时按需注入，不预先进入 System Prompt

> **交叉引用**：当前 Agent 可用的工具范围（白名单/黑名单），由 [agent §F3](agent.md)（Agent 能力组合）定义。本模块负责将可用工具清单渲染为 System Prompt 中的分组描述文本。

### F3. 长期记忆注入

Agent 应能获取跨会话保留的长期记忆内容。长期记忆在主 Agent 会话启动时加载，作为 System Prompt 的组成部分；子 Agent 会话不加载（见 F8 与安全性小节）。长期记忆在每次 System Prompt 组装时从 MEMORY.md 重新读取。
...
System Prompt 的组装触发时机是固定的，组装之间内容不变，利用此不变性做缓存：
...
- 组装事件触发后自动反映最新内容，Owner 无需手动触发生效
