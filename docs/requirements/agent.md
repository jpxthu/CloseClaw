...
配置文件变更后，系统检测到变更并更新配置。权限配置变更即时生效，下一次权限评估即使用新规则（详见 [permission §F6](permission.md)）。修改权限配置不影响 Agent 核心配置，反之亦然。已运行的会话对其他配置的感知由各消费模块自行决定。
...
> **交叉引用**：结果回传的推送机制、消息队列注入、去重保护、sessions_yield 恢复触发等详细行为详见 [session §F4](session.md)（子 Agent 委托与协调）。
...
子 Agent 的权限沿创建链路收窄，不超出任何父 Agent 的权限范围。子 Agent 被拒绝时直接返回权限拒绝错误，不进入用户审批流程（子 Agent 不是面向用户的入口）。
...
> **交叉引用**：系统为每个 Agent-User 组合自动授予的 workspace 路径强制授权是独立机制，详见 [permission §F3](permission.md)（权限决策模型）。
