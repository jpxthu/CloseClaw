...
系统运行时，各模块通过 Agent ID 查询 Agent 的完整配置。查询为只读操作，返回该 Agent 在 F1 定义的全部能力维度——身份标识、模型选择、工作目录、身份加载模式、Bootstrap 文件目录、工具白名单/黑名单、技能白名单、子 Agent 控制参数、记忆配置。查询返回 Agent 的静态配置档案，不包含运行时派生的能力——如权限过滤后的实际工具清单、运行模式决定的工具范围（运行模式定义见 [mode §F1](mode.md)）。
...
- **目标 Agent**：spawn 的目标 Agent，使用其配置档案创建子 Agent（未指定时默认使用当前 Agent 的 ID，即 spawn 一个自己的分身）
- **任务描述**：子 Agent 要完成的任务
- **上下文模式**：子 Agent 仅接收任务描述，还是继承父 Agent 的对话历史（详见「子 Agent 上下文继承（Fork）」）
- **上下文精简**：子 Agent 是否以 Bootstrap 精简模式启动
...
> **交叉引用**：工作目录的强制授权机制（不受 Deny 影响）详见 [permission §F3](permission.md)（权限决策模型）。工作目录的解析顺序由 Session 模块确定。详见 [session §F8](session.md)（工作目录）。父 Agent 可通过显式的拒绝规则覆盖工作目录权限。

### F14. 父子 Agent 通信

子 Agent 创建时，系统自动配置父子 Agent 间的通信路由。父子 Agent 之间通过 spawn 时自动生成的路由互通消息。消息送达需同时满足路由配置和权限允许两个条件。
...
- Agent 配置查询延迟不导致会话创建出现可感知等待
- 每次操作前重新进行权限评估
- 权限配置变更即时生效，无需重启；Agent 配置变更自下次新建会话起生效，无需重启
...
- Agent 配置文件缺失时，系统启动时跳过对应 Agent 的注册，并发出警告
- 权限文件缺失时不阻塞 Agent 加载，使用系统默认权限
- 系统重启后自动恢复 Agent 层级关系，重启前已丢失父会话的子会话降级为根会话继续运行
