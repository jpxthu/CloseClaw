...
每个 Agent 在其自身的工作目录内自动获得读写权限，工作目录内的文件读写不受通用权限规则限制（工作目录豁免优先于链路收窄）。父 Agent 如需限制，可通过显式的拒绝规则覆盖。

> **交叉引用**：工作目录的解析顺序由 Session 模块在 spawn 时确定，详见 [session §F8](session.md)（工作目录）。系统为每个 Agent-User 组合自动授予的 workspace 路径强制授权是独立机制，详见 [permission §F3](permission.md)（权限决策模型）。
