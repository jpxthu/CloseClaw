...
Agent 的身份人格文件通过配置指定，包括身份加载模式和身份人格文件目录。身份加载模式控制上下文注入文件的数量：
- **完整模式**：加载全部身份文件（操作规程、角色定义、身份标识、用户信息、工具指南、自定义引导、长期记忆）
- **精简模式**：仅加载核心文件，减少上下文占用

### F3. Agent 能力组合

Agent 的能力由其配置字段的组合决定，不依赖预定义的类型标签。Agent 的行为边界由配置字段的排列组合决定：
...
- 合并后生成完整配置，所有字段已完成解析——每个字段均已应用合并结果并按需填充默认值
...
配置文件变更后，系统检测到变更并更新配置。已运行的会话是否感知变更由各消费模块自行决定。
...
子 Agent 委托与协调的详细行为见 [session 需求：子 Agent 委托与协调](./session.md#f4-子-agent-委托与协调)。
...
- **Kill**：终止子 Agent 及其所有后代子 Agent，释放其占用的系统资源。子 Agent 的对话历史保留，仍可查阅
- Kill 采用级联清理：从最深层向浅层逐级终止，确保所有后代都被清理，不留下孤立节点

以下场景也触发自动级联清理，子 Agent 对话历史同样保留供查阅：
- 父 Agent 会话正常结束时，所有仍活跃的子 Agent 被自动终止
- 父 Agent 会话被超时清理时，所有子 Agent 被级联终止

> 系统重启恢复时子 Agent 的处理见「Spawn 层级追踪」。
...
- 最高权限身份在权限评估中跳过用户维度，仅受 Agent 配置约束

父 Agent 配置中的拒绝规则沿链路向子 Agent 传播，构成额外的拒绝约束。每增加一级深度，拒绝约束只增不减。权限评估每次操作前重新判定，配置变更即时生效。

权限继承与决策模型的详细行为见 [permission 需求：子 Agent 权限继承](./permission.md#f9-子-agent-权限继承)。
...
Agent 权限配置文件变更后，系统在下一次权限评估时使用新规则，配置变更即时生效。修改权限配置不影响 Agent 核心配置，反之亦然。权限文件不存在时不阻塞 Agent 加载，使用系统默认权限。
...
> 与「持久子 Agent 控制」的关系：持久子 Agent 控制中的级联终止在运行时发生，本节的降级恢复仅在系统重启重建层级关系时生效，两者覆盖不同时间点。
