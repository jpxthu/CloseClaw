...
系统运行时，各模块通过 Agent ID 查询 Agent 的完整配置。查询为只读操作，返回该 Agent 在 F1 定义的全部能力维度——身份标识、模型选择、工作目录、Bootstrap 模式与文件目录、工具白名单/黑名单、技能白名单、子 Agent 控制参数、记忆配置。查询返回 Agent 的静态配置档案，不包含运行时派生的能力（如权限过滤后的实际工具清单、运行模式影响的工具范围）。
